...
// → JSON { fights: [...], runs: [...], raids: [...], days: [...], nights: [...] }  (summarise with a script of your own; see docs/design.md §8.2)
(function () {
  window.__prog = async function (secs, o = {}) {
    ...
    P.beginBattle = function (n) { const r = oBegin.apply(this, arguments); try { const run = this.run; if (run && !run.region.tut && !run.raid) { const mine = M.runPower(run), theirs = M.nodePower(run, n) || 1; this._fi = fights.length; fights.push({ day: this.meta.day, w: run.regionKey, sc: run.scene ? run.scene.i : 0, tier: run.danger || 0, t: n.type + (n.fb ? '*' : ''), col: n.col, seg: n.seg, from: run.chap ? run.chap.from : 0, farm: run.chap && run.chap.farm ? 1 : 0, diff: this.meta.diff || 0, mine, theirs, r: +(mine / theirs).toFixed(2), army: run.roster.length, lv: run.hero.lv }); } } catch (e) {} return r; };
    P.startSettle = function () { const raid = this.run && this.run.raid, b = this.battle; try { if (this._fi != null && fights[this._fi]) { fights[this._fi].res = b.over; fights[this._fi].secs = Math.round(b.t); } this._fi = null; } catch (e) {} const r = oSettle.apply(this, arguments); try { const N = nights[nights.length - 1], res = this.night && this.night.res; if (raid && N && res) Object.assign(N, { res: b.over, secs: Math.round(b.t), fell: res.fell, lost: res.lost, dmg: res.dmg, left: (this.meta.garrison || []).length }); } catch (e) {} return r; };
    ...
    P.raidEnd = function () { const r = this.raid, m = this.meta; try { if (r && !r.done) { const last = raids[raids.length - 1], rec = { day: m.day, over: r.over, portal: +(r.portal.hp / r.portal.max).toFixed(2), secs: Math.round(r.t), kills: r.kills, total: r.total, town: M.raidTown ? M.raidTown(m) : null }; if (last && last.snap && last.over == null && last.day === m.day) Object.assign(last, rec); else raids.push(rec); } } catch (e) {} return oRE.apply(this, arguments); };
