...
window.__bot = async function (secs, opts = {}) {
  ...
  while (performance.now() - T0 < secs * 1000) {
    ...
    try {
      ...
      else if (g.evoFx) { g.evoFx.t = Math.max(g.evoFx.t, 2.7); g.evoFx.go = true; for (let i = 0; i < 90 && g.evoFx; i++) g.tick(1 / 30); }   // an evolution plays (mc-evo.js): let it land, click the card away
