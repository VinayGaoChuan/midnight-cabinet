...
const GIFTS = {
  ...
  talent: { n: '启示卷轴', ic: 'g_scroll', c: '#ffcf4a', w: 9, d: '带着它回来的领袖获得 1 个天赋点。',
    apply(g, m, run) { const h = (run && m.heroes.includes(run.hero)) ? run.hero : M.pick(m.heroes); if (!h) return ok('没有领袖可以读它'); h.points++; return ok(M.qn(h.name, h.rarity) + ' 天赋点 +1', { hero: h.id }); } },
  ...
  calm:   { n: '安神香', ic: 'g_incense', c: '#c8c8ff', w: 8, d: '消除一名领袖的一个坏性格（都没有时，全员回复 25% 生命）。',
    apply(g, m) { const hs = m.heroes.filter(h => h.quirks.some(q => !M.QUIRKS[q].pos)); if (!hs.length) { m.heroes.forEach(h => { h.hp = Math.min(M.heroMaxHp(h, m), h.hp + M.heroMaxHp(h, m) * 0.25); }); return ok('全员回复 25% 生命'); } const h = M.pick(hs), q = M.pick(h.quirks.filter(k => !M.QUIRKS[k].pos)); h.quirks = h.quirks.filter(k => k !== q); return ok(h.name + ' 的「' + M.QUIRKS[q].n + '」消除了', { hero: h.id }); } },
  decoy:  { n: '迷踪粉', ic: 'g_powder', c: '#c8b0ff', w: 8, d: '下一次袭击的怪物减少 30%。',
    apply(g, m) { m.raidWeak = Math.min(0.6, (m.raidWeak || 0) + 0.3); return ok('下一次袭击的怪物 -' + Math.round(m.raidWeak * 100) + '%', { door: 1 }); } },
