...
Object.assign(G, {
  ...
  itemTip(key, q) { const I = M.ITEMS[key]; return { title: I.name, c: M.QUALITY[q || 0].c, kind: '支援道具 · 使用时滚轮决定品质（至少 ' + M.QUALITY[q || 0].n + '）', d: I.desc, lines: I.tiers.map((t, i) => ({ t: M.QUALITY[i].n + '：' + t, c: i >= (q || 0) ? M.QUALITY[i].c : '#5a5460' })) }; },
  relicTip(r) { const R = M.RELICS[r.key]; return { title: R.n, c: M.QUALITY[r.q].c, kind: '宝物 · ' + M.QUALITY[r.q].n, d: R.d, lines: R.lines.map((l, i) => ({ t: M.QUALITY[i].n + '：' + M.statText(l.k, l.v) + (i <= r.q ? '' : '（未达到）'), c: i <= r.q ? M.QUALITY[i].c : '#5a5460' })) }; },
  relicBpTip(key, owned) { const R = M.RELICS[key]; return { title: R.n, c: owned ? '#ffe8b0' : '#8d8496', kind: owned ? '拥有图纸 × ' + owned + ' · 打造花费 ' + this.craftCost() + ' 物资' : '没有图纸', d: R.d + ' 品质越高，能力条目越多：打造出「史诗」品质，就同时拥有普通 + 稀有 + 史诗三条能力。', lines: R.lines.map((l, i) => ({ t: M.QUALITY[i].n + '：' + M.statText(l.k, l.v), c: M.QUALITY[i].c })) }; },
  heroTip(h) { const m = this.meta, H = M.HEROES[h.cls]; return { title: h.name + ' · ' + H.n, c: M.RARITY[h.rarity].c, kind: M.RARITY[h.rarity].n + ' · Lv ' + h.lv + ' · 生命 ' + Math.round(h.hp) + '/' + M.heroMaxHp(h, m), d: '技能「' + H.skill.n + '」' + M.skillDesc(h) + '，冷却 ' + M.skillNodeCd(h, m) + ' 个节点。', lines: h.quirks.map(q => ({ t: '性格 · ' + M.QUIRKS[q].n + '：' + M.QUIRKS[q].d, c: M.QUIRKS[q].pos ? '#9ccc6a' : '#ff6a5a' })).concat(h.status ? [{ t: '疗养中 · 还剩 ' + h.status.days + ' 天', c: '#8d8496' }] : []).concat(h.points ? [{ t: '有 ' + h.points + ' 个天赋点可用', c: '#f2c14e' }] : []) }; },
  ...
  view() {
    ...
    if (v.isBase) {
      ...
      v.b = { day: m.day, raidTxt: raidIn === 0 ? '今晚袭击' : raidIn + ' 天后袭击', raidC: raidIn <= 1 ? '#ff5a4a' : '#e8dcc4', pwTxt: pw.used + ' / ' + pw.made, pwC: pw.free <= 0 ? '#ff6a5a' : '#8ff6ff', portal: Math.round(m.portal.hp) + ' / ' + M.portalMax(m),
        res: [{ k: 'msup', img: M.spriteURL('sack', 4), v: this.tv('msup', m.supplies), c: '#e8c86a', n: '物资', d: '挖掘、建造、打造都要用。' }, { k: 'msh', img: M.spriteURL('shard', 4), v: this.tv('msh', m.shards), c: '#d8a0ff', n: '灵魂碎片', d: '在招魂建筑里招募领袖。' }, { k: 'morb', img: M.spriteURL('orb', 4), v: this.tv('morb', m.orbs), c: '#b8ff9a', n: '经验球', d: '在训练建筑里灌给领袖。' }].map(r => ({ img: r.img, v: r.v, c: r.c, fx: r.k, sc: this.ps(r.k), tipOn: this.tipFn({ title: r.n, c: r.c, d: r.d }) })),
        heroes: m.heroes.map(h => { const mx = M.heroMaxHp(h, m), R = M.RARITY[h.rarity]; return { fx: 'hero-' + h.id, img: M.spriteURL(M.HEROES[h.cls].sprite, 4), name: h.name, lv: h.lv, rc: R.c, hpW: Math.max(0, h.hp / mx * 100) + '%', hpC: h.hp / mx < 0.35 ? '#ff5a4a' : '#9cff7a', dot: h.points > 0, op: h.status ? 0.5 : 1, sc: this.ps('heroes'), tipOn: this.tipFn(() => this.heroTip(h)), onClick: () => { M.Sfx.click(); this.openHero(h.id); } }; }),
        heroCap: m.heroes.length + ' / ' + M.heroCap(m) };
      v.pwTip = this.tipFn({ title: '电力', c: '#8ff6ff', kind: '已用 ' + pw.used + ' / 产出 ' + pw.made, d: '每个房间都要耗电。电力不足时无法建造新的耗电建筑。建造发电类建筑来增加电力。' });
