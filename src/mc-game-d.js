...
Object.assign(G, {
  ...
  relicTip(r) { const R = M.RELICS[r.key]; return { title: R.n, c: M.QUALITY[r.q].c, kind: '', d: R.d, lines: (Array.isArray(r.lines) && r.lines.length ? r.lines : R.lines.slice(0, r.q + 1)).map(l => ({ t: M.statText(l.k, l.v), c: '#e8dcc4' })) }; },   // a made relic says only what it has (2026-09-25)
  ...
  view() {
    ...
    if (v.isBase) {
      ...
      v.b = { day: m.day, raidTxt: raidIn === 0 ? '今晚混沌来袭' : raidIn + ' 天后混沌来袭', raidC: raidIn <= 1 ? '#ff5a4a' : '#e8dcc4', pwTxt: pw.used + ' / ' + pw.made, pwC: pw.free <= 0 ? '#ff6a5a' : '#8ff6ff', portal: Math.round(m.portal.hp) + ' / ' + M.portalMax(m),
        res: [{ k: 'msup', img: M.spriteURL('sack', 4), v: this.tv('msup', m.supplies), c: '#e8c86a', n: '物资', d: '挖掘、建造、打造、招募领袖都要用。' }, { k: 'msh', img: M.spriteURL('shard', 4), v: this.tv('msh', m.shards), c: '#d8a0ff', n: '灵魂碎片', d: '建史诗 / 传说建筑、精铸宝物用。' }].map(r => ({ img: r.img, v: r.v, c: r.c, fx: r.k, sc: this.ps(r.k), tipOn: this.tipFn({ title: r.n, c: r.c, d: r.d }) })),
    ...
    if (s === 'battle' && this.battle) {
      ...
      v.h = { mode: modeN, modeColor: cfg.type === 'boss' || cfg.type === 'elite' ? '#ff6a5a' : cfg.type === 'extract' ? '#5fd0c0' : '#f2c14e', goal, base: M.fmt(this.tv('bbase', b.base)), mult: multR.toFixed(2), multSc: this.ps('mult'), scoreSc: this.ps('bscore'), score: M.fmt(this.tv('bscore', score)),
        ...
        items: run.items.map((k, i) => ({ has: !!k, type: k ? M.ITEMS[k].type || '' : '', img: k ? M.spriteURL(M.ITEMS[k].icon, 7) : '', border: k ? M.ITEM_C : '#3a3040', bg: k ? '#2a1e14' : '#141018', onClick: () => this.useSlot(i), tipOn: this.tipFn(k ? this.itemTip(k, run.itemQ[i]) : { title: '空道具栏' }) })),
