...
class Game {
  ...
  award(list, from) {
    ...
    list.forEach((g, i) => {
      ...
      if (g.k === 'item') { const s = run.items.indexOf(null); if (s < 0) { out.push('道具已满'); return; } run.items[s] = g.key; run.itemQ[s] = g.q || 0; this.hideI.add(s); this.fly(M.ITEMS[g.key].icon, from, 'items', M.QUALITY[g.q || 0].c, () => { this.hideI.delete(s); this.pulse.items = now(); }, d); out.push(M.ITEMS[g.key].name); }
    });
    return out;
  }
  // ── reel ──
