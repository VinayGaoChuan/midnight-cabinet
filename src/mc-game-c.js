...
Object.assign(G, {
  ...
  openEvent(n) {
    ...
    const map = {
      ...
      child: [C('带她走', '可能觉醒好性格', () => { if (Math.random() < 0.6 + luck) { const q = M.addQuirk(h, 1); this.evResult('她把你带到一个藏东西的地方，然后不见了。' + (q ? h.name + ' 觉醒了「' + M.QUIRKS[q].n + '」。' : ''), '#9ccc6a', [{ k: 'rsup', v: 30 }]); } else { const q = M.addQuirk(h, 0); this.evResult('一转身她就不见了。你总觉得背后有人。' + (q ? h.name + ' 染上了「' + M.QUIRKS[q].n + '」。' : ''), '#d0453c'); } }), C('无视', '', () => this.evResult('她一直看着你走远。', '#8d8496'))],
      ...
      clinic: [C('用药', '领袖回复 25% 生命', () => this.evResult('药很苦。回复 ' + this.heroHeal(0.25) + ' 生命。', '#9ccc6a')), C('搜刮', '物资 +，有风险', () => { let t = '你把柜子翻了个遍。'; if (Math.random() < 0.2) { const q = M.addQuirk(h, 0); if (q) t += '你碰了不该碰的东西，染上「' + M.QUIRKS[q].n + '」。'; } this.evResult(t, '#caa84a', [{ k: 'rsup', v: 40 }]); })],
