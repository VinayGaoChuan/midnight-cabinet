...
Object.assign(G, {
  ...
  openEvent(n) {
    ...
    const map = {
      musician: [C('付 ' + fmt(pay) + ' 积分听一曲', '随机好处', () => { run.wallet -= pay; const r = Math.random() + luck; if (r < 0.3) this.evResult('一曲终了，什么也没发生。', '#8d8496'); else if (r < 0.65) { run.runBuff.unitAtk = (run.runBuff.unitAtk || 0) + 0.1; this.evResult('部队听得热血沸腾。本局部队攻击 +10%。', '#f2c14e'); } else { run.runBuff.mult = (run.runBuff.mult || 0) + 0.2; this.evResult('曲子里有好运气。本局初始积分倍率 +0.2。', '#ffcc33'); } }, run.wallet < pay), C('给他一首歌', '免费 · 物资', () => this.evResult('他点点头，从琴盒里拿出一袋东西给你。', '#caa84a', [{ k: 'rsup', v: 15 }])), C('离开', '', () => this.evResult('琴声在你背后停了。', '#8d8496'))],
      ...
      altar: [C('献血', '领袖 -20% 生命，本局积分倍率 +0.5', () => { this.heroHurt(0.2); run.runBuff.mult = (run.runBuff.mult || 0) + 0.5; M.Sfx.mult(); this.evResult('蜡烛亮了一截。本局初始积分倍率 +0.5。', '#ffcc33'); }), C('离开', '', () => this.evResult('烛火跟着你晃了一下。', '#8d8496'))],
  ...
  buy(i) {
    ...
    if (c.kind === 'wheel') { const idx = M.WHEEL.indexOf(M.wpick(M.WHEEL, x => x.w)); this.startReel({ title: '赌博滚轮', iconKey: 'wheel', tiles: M.WHEEL.map(w => ({ n: w.n, sub: '', c: w.c })), land: idx, ups: 0, onDone: () => {
      ...
      if (idx === 3) { run.startMult += 0.3; this.fx.pop(960, 540, '本局初始积分倍率 +0.3', '#ff9a3c', 60); }
