...
G.view = function () {
  ...
  const k = this.roomPanel; if (v.rm.pnOn) {
    ...
    if (k === 'machine') { const m = this.meta, P = M.perks(), active = p.active; Object.assign(pn, { title: '午夜机台', col: '#5fd0c0', img: ic('u_star', 3), sub: active ? '第 ' + (m.gameNo || (p.stats.games + 1)) + ' 局进行中' : '没有进行中的局', what: '坐下，投币。地下基地和异世界探索共同构成一局：基地核心碎掉或主基地被攻破，这一局就结束，回到这个房间结算。',
      ...
      pn.hist = (p.hist || []).slice(0, 5).map(h => ({ t: '第 ' + h.day + ' 天 · ' + (h.reason === 'core' ? '基地爆炸' : '主基地被攻破') + ' · +' + h.tokens + ' 代币' })); pn.hasHist = pn.hist.length > 0; }
  ...
  if (v.rm.stOn) { const P = p.pending; if (!P.shownAt) P.shownAt = t0; const e = (t0 - P.shownAt) / 1000, rows = P.rows || [];
    v.rm.st = { title: P.reason === 'core' ? '基地爆炸' : '主基地被攻破', col: '#ff5a4a', sub: '这一局撑到了第 ' + P.day + ' 天。机台吐出了一把代币。', rows: rows.map((r, i) => { const q = cl((e - 0.4 - i * 0.22) / 0.3, 0, 1); return { img: ic(r.ic, 2), n: r.n, v: '×' + r.v, t: '+' + Math.round(r.t * q), op: q, dy: Math.round((1 - q) * 20), dim: r.v ? 1 : 0.5 }; }),
