...
// From the slam to the end of the effect the fight stands still (mc-fever.js). After the reel the effect plays over the
// field, whatever it brings flies in glowing; the moment its last blow lands the fight goes on and FEVER TIME runs (its
// +0.2 flies into the multiplier as two chips). 2026-09-26 (user ruling): 「Fever有一个全屏压暗，这个效果取消」 — no dark
// screen any more; 「效果触发结束后立刻结算」 — no tail of shaking after the last blow.
...
// (稀有 chains, 史诗 adds the shockwave, 传说 stuns / revives / turns the enemy); 神话 does all of it and more, in red
const FX = M.FEVER_FX = {
  bolt: { n: [2, 4, 6, 9, 12, 16], dmg: [0.6, 0.75, 0.9, 1.4, 2.2, 3.0] },
...
if (I) {
  Object.assign(I.bolt, { tiers: ['2 道落雷', '4 道落雷', '6 道落雷，会连锁', '9 道粗雷 + 冲击波', '12 道紫雷，劈晕敌人', '16 道血雷：连锁、冲击波、劈晕'] });
...
G.fvStageStart = function (key, tier, b) {
  ...
  if (key === 'bolt') {
    ...
    for (let i = 0; i < n; i++) on(0.5 + i * gap, () => {
      ...
      b.deal(b.hero, tg, dmg, { skill: 1, col: '#e0e8ff', big: f >= 2 }); g.fx.pop(p.x, p.y - 70, M.fmt(Math.round(dmg)), myth ? P.red : f >= 3 ? P.violet : '#e0e8ff', 46 + tier * 5, { rise: 60 });
      if (!RM() && g.fx.trauma < 0.45) g.fx.kick(3 + tier); g.fx.flash(myth ? '#ffc0c8' : f >= 3 ? '#e0c0ff' : '#fff8d8', 0.12 + f * 0.06); S.bolt && S.bolt(f);
      if (f === 1 || myth) { const o = F.filter(x => x !== tg).sort((a, c) => Math.hypot(a.x - tg.x, a.y - tg.y) - Math.hypot(c.x - tg.x, c.y - tg.y))[0]; if (o && Math.hypot(o.x - tg.x, o.y - tg.y) < 300) { const q = bodyAt(g, o); st.arcs.push({ x1: p.x, y1: p.y, x2: q.x, y2: q.y, t0: st.t }); b.deal(b.hero, o, dmg * 0.5, { skill: 1, col: '#e0e8ff' }); st.holes.push({ e: o, r: 90, t0: st.t, life: 0.7 }); } }
      if (f >= 2) { const r = f === 3 ? (myth ? 230 : 190) : 130; g.fx.ring(p.x, p.y + 30, 20, r * zoom(g), myth ? P.red : f === 3 ? P.violet : P.gold, 10, 0.45);
    ...
    A.forEach((o, i) => on(0.6 + i * 0.07, () => st.flyers.push({ kind: 'mote', from: C, e: o, t0: st.t, dur: 0.45, col: '#b8ffb0', land: () => {
      const h0 = o.hp; b.heal(o, o.maxHp * pct); if (f >= 2) { o.shield = o.maxHp * 0.3; st.marks.push({ e: o, kind: 'shield', t0: st.t }); }
    ...
    if (myth) on(tEnd, () => { A.forEach(o => { if (o.alive) { o.atk *= 1.3; st.holes.push({ e: o, r: 95, t0: st.t, life: 99 }); } }); const p0 = { x: 960, y: 420 }; g.fx.pop(p0.x, p0.y, '部队攻击 +30%', P.red, 44, { rise: 60 }); if (!RM()) g.fx.kick(10); });
    if (f === 3) { const dead = b.ents.filter(o => !o.alive && o.side === 'A' && !o.isHero && !o.summon && !o.evolved);
    ...
    st.end = 0.55 + (list.length - 1) * 0.4 + 0.6 + 0.3;
  } else if (key === 'bell') {
...
// the fx layer goes over the bar while the stage plays (the title and the lit multiplier sit on it)
const oView = G.view;
...
function drawStage(ctx, g, st) {
  ...
  st.bolts.forEach(bo => { const q = (T - bo.t0) / 0.3; if (q < 0 || q > 1) return; const w = [8, 10, 14, 18][bo.tier] + (bo.myth ? 6 : 0); ctx.save(); ctx.globalAlpha = 1 - q; jag(ctx, bo.x + ((bo.seed % 200) - 100), 0, bo.x, bo.y, bo.seed, w + 10, bo.myth ? P.red : bo.tier >= 3 ? P.violet : '#8fb8ff'); jag(ctx, bo.x + ((bo.seed % 200) - 100), 0, bo.x, bo.y, bo.seed, w * 0.45, P.white); ctx.restore(); M.glow(ctx, bo.x, bo.y, 180, bo.myth ? P.red : bo.tier >= 3 ? P.violet : '#e0e8ff', 0.7 * (1 - q)); });
