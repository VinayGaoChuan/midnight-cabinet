...
M.wouldMerge = () => false;
M.sellValue = (run, u) => Math.round((DB[u.type].cost || 10) * 0.5 * (u.ek || 1));   // an evolved unit sells for what it is worth (mc-evo.js)
M.synergies = () => ({ cnt: {}, lvl: {} });
M.TAGS = {};
// ───────── banners (legion) ─────────
M.LEGION = {
...
M.makeBattleCfg = function (run, node) {
  ...
  if (mode === 'hold') {
    ...
    for (let i = 0; i < waves; i++) M.pickWave(budget * (t === 'extract' ? M.EXTRACT_WAVE : 0.45)).forEach((e, j) => cfg.list.push(Object.assign(e, { spawn: 1.6 + i * 8 + j * 0.35 })));
  } else {
