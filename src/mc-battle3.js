...
class B3 extends M.Battle2 {
  ...
  unitStats(key, side, x, y, o = {}) {
    ...
    const ek = side === 'E' ? this.ek * (o.elite ? 1.15 : 1) : (u && u.ek > 1 ? u.ek : 1);   // an evolved unit's life and attack (mc-evo.js)
  ...
  step(dt) {
    ...
    for (const e of act) {
      ...
      const dx = tg.x - e.x, dy = (tg.y - e.y) * (tg.wideY || 1), d = Math.hypot(dx, dy);   // wideY: a final boss is fought along the whole edge of its arena (mc-bossfight.js)
  ...
  render(ctx, opts = {}) {
    ...
    if (M.drawAfterUnits) M.drawAfterUnits(ctx, this, T);
