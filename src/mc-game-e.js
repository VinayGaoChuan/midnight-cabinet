...
Object.assign(G, {
  panelView() {
    ...
    if (p.kind === 'room') {
      ...
      if (pn.isRecruit) { pn.recTxt = '花费 100 灵魂碎片，从另一个世界招来一名领袖。品质随机：黑铁 / 青铜 / 白银 / 黄金，天赋树也随机。' + (B.recruit.qUp ? '（巨石阵：至少青铜）' : ''); pn.recBtn = '招魂 · 100 碎片（现有 ' + m.shards + '）'; pn.recOk = m.shards >= 100 && m.heroes.length < M.heroCap(m); }
