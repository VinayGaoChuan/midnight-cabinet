...
class Battle {
  ...
  castSkill(free) {
    ...
    this.pending.push({ t: this.t + 0.45, fn: () => {
      ...
      if (cls === 'widow') { this.allin = this.t + 8; this.allinV = v; for (let i = 0; i < 60; i++) this.fx.push({ k:'coin', x:200 + Math.random() * 1700, y:-40 - Math.random() * 300, vy:500 + Math.random() * 400, t0:this.t + Math.random() * 0.6, life:1.4 }); this.float(960, 200, '梭哈！击杀积分倍率 +' + v.toFixed(2), C.gold, 64); }
