...
class Battle {
  ...
  castSkill(free) {
    ...
    this.pending.push({ t: this.t + 0.45, fn: () => {
      ...
      if (cls === 'widow') { this.allin = this.t + 8; this.allinV = v; for (let i = 0; i < 60; i++) this.fx.push({ k:'coin', x:200 + Math.random() * 1700, y:-40 - Math.random() * 300, vy:500 + Math.random() * 400, t0:this.t + Math.random() * 0.6, life:1.4 }); this.float(960, 200, '梭哈！击杀积分 ×' + (1 + v * 10).toFixed(1), C.gold, 64); }
      ...
      if (cls === 'butcherlord') { const al = this.ents.filter(o => this.active(o) && o.side === 'A' && !o.isHero); if (al.length) { const vv = al.reduce((a, b) => (b.hp < a.hp ? b : a)); this.fx.push({ k:'boom', x:vv.x, y:vv.y - 30, t0:this.t, life:0.5 }); this.burst(vv.x, vv.y - 40, '#ff2a2a', 40); this.kill(vv, null, false); const up = v * 0.25; this.ents.forEach(o => { if (o.alive && o.side === 'A' && !o.isHero) { o.atk *= 1 + up; this.burst(o.x, o.y - 40, '#ff3a3a', 6); } }); this.float(vv.x, vv.y - 140, '血祭 · 全队攻击 +' + Math.round(up * 100) + '%', '#ff6a6a', 40); this.ring(vv.x, vv.y - 30, 20, 600, '#ff3a3a', 16, 0.6); } else this.float(960, 200, '没有可献祭的部队', C.dim, 40); }
  ...
  addMult(m, x, y, label) { m = Math.round(m * 100) / 100; this.mult += m; this.float(x, y, (label ? label + ' ' : '') + '积分倍率 +' + m, C.gold, 40); Sfx.mult(); }
  kill(e, src, crit) {
    if (!e.alive) return;
  ...
  render(ctx, opts = {}) {
    ...
    if (!M.LOW_FX) lights.forEach(L => { if (L.r > 0 && L.c && L.c.length === 7) M.glow(ctx, L.x, L.y, L.r * 0.5, L.c, 0.18); });
