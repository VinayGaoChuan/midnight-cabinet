...
M.RAID_K = 1; M.RAID_BOSS = 0.8;
const OldRaid = M.Raid;
M.Raid = class extends OldRaid {
  constructor(meta) { super(meta); const bud = M.budgetAt(1 + meta.day * 0.32 + meta.raids * 0.4); this.list = []; for (let w = 0; w < 3; w++) M.pickWave(bud * 0.5).forEach((e, i) => this.list.push({ t: 2 + w * 9 + i * 0.6, type: e.type, side: Math.random() < 0.5 ? -1 : 1 })); if (meta.day >= 10) { const bt = meta.day >= 20 ? 'ChaosGuardBlackKatos' : 'Kraken', E = DB[bt]; this.list.push({ t: 30, type: bt, side: 1, sc: E && E.cost ? Math.min(1, bud * M.RAID_BOSS / E.cost) : 1 }); } /* one leader (2026-09-25): the raid boss is sized to the day's budget */ this.total = this.list.length; this.ents.forEach(e => { if (e.sprite === 'nail') { e.sprite = 'FootSoldier'; e.hp = e.max = DB.FootSoldier.hp * (1 + meta.day * 0.1); e.atk = DB.FootSoldier.atk * 2; } }); }
