...
function drawRoom(ctx, X, Y, W, H, key, t, o = {}) {
  ...
  if (B.q >= 3) { const gw = B.q >= 4 ? 34 : 24; [[0, 0, w, gw, 0, 0, 0, gw], [0, h - gw, w, gw, 0, h, 0, h - gw], [0, 0, gw, h, 0, 0, gw, 0], [w - gw, 0, gw, h, w, 0, w - gw, 0]].forEach(([a, b, c, d, x0, y0, x1, y1]) => { let g; if (M.UI) g = M.UI.lg(ctx, x0, y0, x1, y1, [[0, qc], [1, 'rgba(0,0,0,0)']], 3); else { g = ctx.createLinearGradient(x0, y0, x1, y1); g.addColorStop(0, qc); g.addColorStop(1, 'rgba(0,0,0,0)'); } al(0.32 * qa); ctx.globalCompositeOperation = 'lighter'; ctx.fillStyle = g; ctx.fillRect(a, b, c, d); ctx.globalCompositeOperation = 'source-over'; }); al(1);
    if (B.q >= 4) for (let i = 0; i < 6; i++) { const q = (t * 0.35 + i / 6) % 1, per = 2 * (w + h), d0 = q * per, px = d0 < w ? d0 : d0 < w + h ? w - 7 : d0 < 2 * w + h ? w - (d0 - w - h) : 7, py = d0 < w ? 7 : d0 < w + h ? d0 - w : d0 < 2 * w + h ? h - 7 : h - (d0 - 2 * w - h); spark4(ctx, px, py, 6, '#ffffff'); glowC(ctx, px, py, 14, qc, 0.8); } }
