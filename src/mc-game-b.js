...
Object.assign(G, {
  ...
  baseClick(sx, sy) {
    ...
    else { this.toast('只能挖掘与已有房间相邻的岩层', '#8d8496'); }
  },
  openPanel(o) { this.panel = Object.assign({ at: now() }, o); M.Sfx.whoosh(0.18); this.fx.spark(1190, 560, '#ffd970', 16, { dir: Math.PI, spread: 1.4, v: 900, w: 3, g: 0 }); this.fx.flare(1190, 560, 220, '#ffd970', 0.25); this.fx.kick(2); this.bump(); },
  ...
  doBuild(c, r, key) {
    ...
    if (!o) return; if (o.why) { this.toast(o.why, '#d0453c'); return; }
    this.hold('msup', m.supplies); M.startBuild(m, c, r, key); this.release('msup');
    const p = this.cellPos(c, r); M.Sfx.build(); M.Sfx.up(1); this.fx.explode(p.x, p.y, M.QUALITY[o.B.q].c, 1.2 + o.B.q * 0.5); this.fx.pop(p.x, p.y - 70, o.B.n + ' 开工 · ' + o.days + ' 天', M.QUALITY[o.B.q].c, 44);
  ...
  craft(key, c, r) {
    ...
    if (!M.invHas(m, 'rbp:' + key)) { this.toast('没有「' + R.n + '」的图纸', '#8d8496'); return; }
    if (m.supplies < cost) { this.toast('物资不足', '#d0453c'); return; }
    ...
    this.startReel({ title: '打造 · ' + R.n, iconKey: R.icon, itemMode: true, land: 0, ups: res.landQ, tease: res.landQ < 3, tiles: M.QUALITY.map((q, i) => ({ n: q.n, sub: M.statText(R.lines[i].k, R.lines[i].v), c: q.c })), onDone: () => {
      ...
      this.fx.rays(960, 460, col, 2.2, { r: 700 }); this.fx.pop(960, 700, M.QUALITY[res.r.q].n + ' · ' + R.n, col, 80, { life: 1.8, rise: 20 }); M.Sfx.sparkle();
  ...
  train(id, c, r) {
    ...
    if (!m.orbs) { this.toast('没有经验球', '#8d8496'); return; }
  ...
  quickHeal(id) {
    ...
    if (h.hp >= mx) return; if (m.supplies < 40) { this.toast('物资不足', '#d0453c'); return; }
  ...
  launch() {
    ...
    setTimeout(() => { this.fx.flash('#e0fff5', 1); this.run = M.newRun3(m, h, p.world, p.relics); this.enterWorld(); this.toast(this.run.region.n + ' · 长度「' + this.run.len.n + '」· ' + this.run.map.cols + ' 站', '#f2c14e'); }, 750);
  },
