...
MINI.musician = { title: '流浪乐师', img: 'musician', col: C.gold, text: '没有脸的乐师拉着琴。琴声停下来，他把弓递给了你。',
  ...
  hit(mg, l) { if (mg.phase !== 'play') return; mg.laneF[l] = 1; let best = null, bd = 0.24; mg.notes.forEach(n => { if (n.st || n.l !== l) return; const d = Math.abs(n.t - mg.song); if (d < bd) { bd = d; best = n; } });
    ...
    mg.fly.push({ x: LANE_X[l], y: HIT_Y, t: 0, txt: best.st === 2 ? '完美' : '不错', c: best.st === 2 ? C.gold : C.green }); if (best.st === 2) this.fx.spark(LANE_X[l], HIT_Y, '#ffe08a', 8, { dir: -Math.PI / 2, spread: 1.2, v: 500 }); },
  ...
  draw(x, mg) {
    ...
    LANE_X.forEach((lx, i) => { x.fillStyle = K.LG(x, 0, SY + 90, 0, HIT_Y, [[0, 'rgba(255,224,138,0)'], [1, 'rgba(255,224,138,' + (0.12 + mg.laneF[i] * 0.4) + ')']]); x.fillRect(lx - 70, SY + 90, 140, HIT_Y - SY - 60); K.R(x, lx - 70, SY + 90, 2, HIT_Y - SY - 60, 'rgba(255,224,138,0.2)'); K.R(x, lx + 68, SY + 90, 2, HIT_Y - SY - 60, 'rgba(255,224,138,0.2)');
      ...
      const f = mg.laneF[i], hs = 40 + Math.ceil(f * 2) * 4; K.R(x, lx - 44, HIT_Y - 44, 88, 88, 'rgba(7,6,15,0.6)'); K.RR(x, lx - hs - 3, HIT_Y - hs - 3, hs * 2 + 6, hs * 2 + 6, 0, null, f ? C.butter : C.gold, 6); U.key(x, ['Q', 'W', 'E'][i], lx - 24, HIT_Y + 46, { size: 24 }); });
    const sp = (HIT_Y - SY - 90) / TRAVEL;
    mg.notes.forEach(n => { if (n.st > 0 || n.st === -1 && mg.song - n.t > 0.6) return; const y = HIT_Y - (n.t - (mg.song || -9)) * sp; if (y < SY + 60 || y > HIT_Y + 80) return; const col = [C.pink, C.ice, C.lime][n.l]; x.globalAlpha = n.st === -1 ? 0.3 : 1; K.GL(x, LANE_X[n.l], y, 50, col, 0.6); K.CI(x, LANE_X[n.l], y, 30, col); K.IC(x, 'e_music', LANE_X[n.l], y, 40); x.globalAlpha = 1; });
    mg.fly.forEach(f => { x.save(); x.globalAlpha *= 1 - Math.floor(f.t / 0.7 * 4) / 4; K.big(x, f.txt, f.x, f.y - 60 - Math.floor(f.t * 80 / 6) * 6, T.item, f.c, f.t); x.restore(); });
...
MINI.granny = { title: '裁缝老太', img: 'old', col: C.violet, text: '她能把一名部队缝进你的影子里，让你的伤口合上。针脚越齐，伤好得越快。',
  ...
  draw(x, mg) {
    ...
    if (mg.phase === 'sew') { x.strokeStyle = U.pal('rgba(208,160,255,0.35)'); x.lineWidth = 3; x.setLineDash([12, 10]); x.beginPath(); for (let u = 0; u <= 1.001; u += 0.02) { const p = MINI.granny.path(u); u ? x.lineTo(p.x, p.y) : x.moveTo(p.x, p.y); } x.stroke(); x.setLineDash([]);
      // 针脚点：方块，白 / 紫圈 + 墨边，落针后打叉
      mg.marks.forEach(m => { const p = MINI.granny.path(m.u), px = Math.round(p.x), py = Math.round(p.y); U.box(x, px - 18, py - 18, 36, 36, m.ok ? C.white : C.violet); K.R(x, px - 15, py - 15, 30, 30, m.ok ? C.violet : C.abyss); if (m.ok) { K.LN(x, px - 9, py - 9, px + 9, py + 9, 3, C.white); K.LN(x, px + 9, py - 9, px - 9, py + 9, 3, C.white); } });
      const u = cl(mg.pt / 4, 0, 1), p = MINI.granny.path(u); x.strokeStyle = C.red; x.lineWidth = 3; x.beginPath(); for (let v = 0; v <= u; v += 0.01) { const q = MINI.granny.path(v); v ? x.lineTo(q.x, q.y - 4) : x.moveTo(q.x, q.y - 4); } x.stroke(); x.save(); x.translate(p.x, p.y); x.rotate(-0.8); K.IC(x, 'e_needle', 0, -20, 70); x.restore(); U.text(x, mg.good + ' / 6', CX, SY + 170, T.num, C.violet, { num: true, outline: true }); }
  } };
...
MINI.well = { title: '许愿井', img: 'well', col: C.teal, text: '井底有东西在回应你的脚步声。硬币扔得越准，回应越慷慨。',
  ...
  draw(x, mg) {
    ...
    if (mg.phase === 'fly') { const q = cl(mg.pt / 0.95, 0, 1), p = traj(mg.shot, q); if (q < 1) { x.save(); x.translate(p.x, p.y); x.scale(Math.abs(Math.cos(mg.pt * 18)) * 0.8 + 0.2, 1); K.IC(x, 'e_coin', 0, 0, 40); x.restore(); } else if (mg.acc > 0.2) { for (let i = 0; i < 3; i++) { x.strokeStyle = U.pal('rgba(143,224,255,' + cl(1 - (mg.pt - 0.95) / 0.8, 0, 1) + ')'); x.lineWidth = 3; x.beginPath(); x.ellipse(wx, wy - 20, 20 + (mg.pt - 0.95) * 120 * (1 + i * 0.4), 6 + (mg.pt - 0.95) * 30, 0, 0, 7); x.stroke(); } } }
...
MINI.grave = { title: '无名墓碑', img: 'tomb', col: C.lavender, text: '墓碑上没有名字，土是新翻的。蜡烛烧完之前挖到底。',
  ...
  draw(x, mg) {
    ...
    if (mg.phase === 'out') K.R(x, SX, SY, SW, SH, 'rgba(7,6,15,' + cl(Math.floor(mg.pt * 5) / 5, 0, 0.85) + ')');
  } };

// ═════════════════════ 废弃医务室 · pick bottles off the shelf ═════════════════════
const MEDS = [
  { n: '绿色药水', c: C.green, d: '回血', f(g) { return '回复 ' + g.heroHeal(0.2) + ' 生命'; } },
  { n: '蓝色药水', c: C.blue, d: '经验', f(g, mg) { return g.giveExp(45, mg.from); } },
  { n: '金色药水', c: C.gold, d: '宝贝', f(g, mg) { return g.award([K.item(g.run, mg.P)], mg.from).join(''); } },
  { n: '白色药片', c: C.white, d: '物资', f(g, mg) { return g.award([{ k: 'rsup', v: 25 }], mg.from).join(''); } },
  { n: '红色药水', c: C.red, d: '剧毒', f(g) { return '中毒 -' + g.heroHurt(0.1); } },
  { n: '紫色药水', c: C.violet, d: '倍率', f(g) { g.run.runBuff.mult = (g.run.runBuff.mult || 0) + 0.2; return '本局初始倍率 +0.2'; } }];
MINI.clinic = { title: '废弃医务室', img: 'gurney', col: C.ice, text: '药柜里有六个瓶子。有两个标签被血糊住了。你最多敢试三瓶。',
  ...
  draw(x, mg) {
    ...
    mg.bt.forEach(b => { const hov = mg.phase === 'idle' && !b.opened && Math.abs(mg.mx - b.x) < 55 && Math.abs(mg.my - b.y) < 90, lift = hov ? 14 : 0, col = b.dark && !b.opened ? '#2a1a1a' : b.m.c;
      ...
      K.RR(x, -34, -110, 68, 110, 14, 'rgba(200,230,255,0.25)', C.silver, 3); K.RR(x, -30, -70, 60, 66, 10, col); if (!b.dark || b.opened) K.GL(x, 0, -40, 60, col, 0.3); K.R(x, -12, -130, 24, 22, C.brown); if (b.dark && !b.opened) { K.R(x, -26, -64, 52, 30, C.wine); U.text(x, '?', 0, -50, T.item, C.cream); } K.R(x, -24, -100, 8, 60, 'rgba(255,255,255,0.35)');
      if (b.open > 0 && !b.done) { K.R(x, -12, -130 - b.open * 60, 24, 22, C.brown); } x.restore(); });
  } };
...
MINI.mirror = { title: '落地镜', img: 'mirror', col: C.blue, text: '镜子里的你慢了半拍才动。它在示范什么——跟着做一遍。',
  ...
  draw(x, mg) {
    ...
    x.save(); x.beginPath(); x.rect(mx - 150, my - 240, 300, 420); x.clip(); pose(mg.mpose || (mg.phase === 'input' || mg.phase === 'win' ? mg.pose : null), mx, my + 150, true, 1); K.R(x, mx - 150, my - 240, 300, 420, 'rgba(80,120,200,0.25)'); if (mg.phase === 'fail') K.R(x, mx - 150, my - 240, 300, 420, 'rgba(232,67,79,' + cl(Math.floor(mg.pt * 4) / 4, 0, 0.5) + ')'); x.restore();
...
// ═════════════════════ 血祭坛 · hold to pour ═════════════════════
MINI.altar = { title: '血祭坛', img: 'candle', col: C.red, text: '一滴血，一分运。倒得越多，倍率越高——但蜡烛随时可能被血浇灭。',
  ...
  draw(x, mg) {
    ...
    // 倍率读数：品红大字，每涨一档弹一下；流血量红字
    K.big(x, '倍率 +' + (Math.floor(mg.poured / 0.04) * 0.1).toFixed(1), CX, SY + 160, T.num, C.magenta, mg.phase === 'pour' ? (mg.poured % 0.04) / 0.1 : 9); U.text(x, '已流血 ' + Math.round(mg.poured * 100) + '%', CX, SY + 215, T.body, C.red);
...
MINI.peddler = { title: '货郎', img: 'stall', col: C.violet, text: '货郎掀开布：「三个瓶子，一个有货。眼睛跟得上，东西归你。」',
  ...
  btns(mg) { if (mg.phase === 'idle') { const over = mg.rounds >= 2, buy = M.nice(mg.pay * 1.5); return [{ t: '押一局', sub: mg.pay + ' 积分 · 猜中得高品质道具', gold: !over, dis: over || this.run.wallet < mg.pay || this.run.items.indexOf(null) < 0, why: over ? '货郎不跟你玩了' : this.run.items.indexOf(null) < 0 ? '道具栏满了' : '积分不够', fn: () => MINI.peddler.start.call(this, mg) }, { t: '直接买', sub: buy + ' 积分 · 随机道具', dis: this.run.wallet < buy || this.run.items.indexOf(null) < 0, why: '积分不够或道具栏满了', fn: () => { if (!this.miniPay(buy)) return; this.miniFinish('货郎把瓶子塞到你手里。', '#b86bff', [K.item(this.run, mg.P)]); } }, { t: '离开', leave: 1, fn: () => this.miniFinish(mg.got.length ? '你赢走了：' + mg.got.join('、') + '。' : '货郎把布盖了回去。', mg.got.length ? '#b86bff' : '#8d8496') }]; }
  ...
  draw(x, mg) {
    ...
    [0, 1, 2].forEach(id => { const s = mg.slot.indexOf(id), lx = cupX[id], ly = SY + 430 + (arc[id] || 0), lift = mg.lift[s] || 0, hov = mg.phase === 'guess' && Math.abs(mg.mx - lx) < 90 && Math.abs(mg.my - (SY + 380)) < 120;
      ...
      x.save(); x.translate(lx, ly - lift * 120 - (hov ? 10 : 0)); K.PL(x, [[-60, 0], [60, 0], [40, -150], [-40, -150]], C.violetDeep); K.PL(x, [[-50, -10], [-20, -10], [-24, -140], [-34, -140]], 'rgba(255,255,255,0.15)'); K.R(x, -20, -176, 40, 30, C.brown); U.box(x, -62, -8, 124, 12, C.violet); if (hov) K.GL(x, 0, -80, 140, C.violet, 0.4); x.restore(); });
