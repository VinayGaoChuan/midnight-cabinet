...
// ───────── snapshots ─────────
const heroS = (h, m) => h && ({ cls: h.cls, lv: h.lv, r: h.rarity, hp: r2(h.hp / Math.max(1, M.heroMaxHp(h, m))), tal: Array.isArray(h.taken) ? h.taken.length : 0 });
...
wrap(G, 'takeTalent', function (id, i) { const h = this.meta.heroes.find(x => x.id === id), n = h && h.tree[i]; return n ? { cls: h.cls, f: n.f, L: n.L, lv: h.lv, pts: h.points } : null; }, function (c) { if (c) T.ev('talent', c); });
