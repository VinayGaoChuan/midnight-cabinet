...
| `mc-flow.js` | 流程：开场动画 → 标题菜单 → 基地（房间系统屏蔽时）；基地的「放弃」按钮 |
| `mc-tipcompact.js` | 悬浮说明、卡牌、地形、面板“图标为主”：平时只显示图标行，按住 Ctrl 展开文字 |
| `mc-guide.js` | 文字分层与说明：按住 Ctrl 看详情、每个设计元素的初见说明卡、玩法说明页（F1 / 右下角「？」） |
