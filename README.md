...
| `tools/sim-power.js` | 无画面批量战斗：`node tools/sim-power.js 3000 .ai/sim-rows.json`，检验战斗力比值能不能预测胜负（标定用） |
...
| `mc-pskill.js` | 领袖技能（每个职业一个）：场外按空格释放、按节点冷却；亲自上场 / 守城时自动释放（怒气、能量、法力或秒数充能，范围里有敌人才放） |
...
| `mc-skilltrigger.js` | 技能触发条件：带法力特性的部队开战时法力满，每个特性（和领袖个人技能）满足自己的条件才放 |
| `mc-awaken.js` | 部队：145 条特性的一句话和激活类别；悬浮说明与夜市卡片（职业 + 战斗力 + 一句话）；开战激活演出、头顶特性图标、亡语图标飞向击杀者 |
| `mc-portal.js` | 传送门：点开升起世界碑（危险度 + 特产），选碑后碑砸进门里、传送门变成该世界的样子；危险度按领袖战斗力算 |
| `mc-vision.js` | 地图视野：默认 2 步，按基地、地格、世界、领袖天赋、奇遇增减；顶栏眼睛图标 |
| `mc-recruit.js` | 招募的碎卡演出；招募房间的「扩建」（领袖上限 +1） |
| `mc-power.js` | 战斗力：√(总生命 × 总秒伤)，部队 / 领袖 / 整队 / 每场战斗同一把尺子；地图上敌人头上、领袖头上的战斗力徽章；传送门碑的危险度 |
| `mc-flow.js` | 流程：开场动画 → 机台屏幕上的菜单 → 钻进屏幕 → 基地（房间系统屏蔽时）；基地的「放弃」按钮 |
| `mc-tipcompact.js` | 悬浮说明、卡牌、地形、面板“图标为主”；领袖的文字按住 Ctrl 展开，其余直接显示 |
| `mc-guide.js` | 文字分层与说明：领袖按住 Ctrl 看详情、每个设计元素的初见说明卡、玩法说明页（F1 / 右下角「？」） |
