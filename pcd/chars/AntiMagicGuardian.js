// 破魔守卫（部队 · 兽人 · 先锋 · 传说）：魔盾兵升级后的同一个兽人——青铜符文塔盾长成秘银破魔塔盾（金框、外侧两片金边翼片、獠牙护角包金、
...
PCD.define('AntiMagicGuardian', (E) => {
  ...
  const HX = 73, DUR = DEFAULT_DUR.slice();
  ...
  const K_SLUMP = K(3, -12, 1, -11, 1, -1, 3);       // 死亡：盔甲撑不住往前一沉、往下一坐（头盔离地低一些，散架后落得快）
  ...
  const KEY2 = parts.keyer([['step', -1, 1], ['wup', 0, 2], ['walk', 0, 1], ['beard', -3, 3], ['sway', -2, 2], ['gem', 0, 4], ['lid', 0, 2], ['pupil', -1, 1], ['halo', 0, 11],
    ['ring', 0, 11], ['core', 0, 3], ['drop', 0, 6], ['tilt', 0, 1], ['rim', 0, 3], ['eyes', 0, 1], ['mist', 0, 1], ['flash', 0, 1], ['dq', 0, 48, 48], ['bx', -16, 15], ['st', 0, 8]]);
  const PUPIL = [0, 1, 1, 0, -1, -1];                                   // 待机个性：破魔之眼左右扫视（每 0.4 s 一格）
  const T_PUSH = 2 / 12, T_FLIP = 4 / 12, T_SWEEP = 0.15, T_BREAK = INCOMING + 0.72;
  // 散架（死亡套件 parts）：力从头顶上方往下压（fromY −34），空甲往下塌、往身后散，头盔不往上抛；
  //   碎甲在 1.39 s 前全部着地、1.59 s 前全部停稳，2.22 s 开始消散、2.82 s 消散完（静止段 0.63 s）
  const DK_OPT = { power: 0.55, fromX: 2, fromY: -34, push: -6, fadeAt: 1.2, fadeDur: 0.6 };
  const T_CLANK = T_BREAK + 0.36, HELM_LX = -7;                            // 最后一块大件（整顶头盔）着地的那一帧，和它落地的位置（本地 x）
  ...
  function poseAt(st, t, T) {
    ...
    P.halo = Math.floor(TT * 5 + 1e-6) % 12; P.ring = Math.floor(TT * 5 + 1e-6) % 12;   // 2.4 s：石板环转 60°（6 块对称）、肩环转 90°（紫点 4 个对称），正好接上
    const idle = () => {
    ...
    if (st === IDLE) idle();
      ...
      P.halo = Math.floor(tq * 5 + tq * tq * 10 + 1e-6) % 12; P.ring = Math.floor(tq * 6 + tq * tq * 12 + 1e-6) % 12;
      ...
      P.halo = Math.floor(tq * 30 + 1e-6) % 12; P.ring = Math.floor(tq * 24 + 1e-6) % 12;
      ...
      P.halo = Math.floor(tq * 12 - tq * tq * 6 + 1e-6) % 12; P.ring = Math.floor(tq * 10 - tq * tq * 5 + 1e-6) % 12;
    } else if (st === HURT) {
      const h = tq - INCOMING;
      if (h < 0) idle();
      else if (h < 0.2) { setK(K_HURT, K_HURT, 0); P.bx = -2; P.eyes = 1; P.lid = 2; P.flash = h < 1 / 12 ? 1 : 0; P.halo = (P.halo + 2) % 12; P.ring = (P.ring + 2) % 12; }   // 符文环被震得跳一格
  ...
  //   三根尖刺：顶上一根朝上，两层外端各一根朝外上。肩上套一个转动的符文环（横椭圆 12 个点，紫色符文点隔 2 个金点一个）：后半圈在 ringBack 里先画，前半圈和肩甲同一个部件
  //   后肩用竖叠版 pauldronBack（三层往下叠、只往上伸一根尖刺），不往身后横伸，免得读成一只往后伸的手
  const RUNE_DOT = (k) => k % 3 === 0;
  function ringDots(x, y, front, cb, rx) {
    rx = rx || 6;
    for (let k = 0; k < 12; k++) { const a = (k * 30 + P.ring * 7.5) * Math.PI / 180, s = Math.sin(a); if ((s >= 0) !== front) continue; cb(x + RD(Math.cos(a) * rx), y + RD(s * 2), k); }
  }
  // own 0 = 不另起部件（画进当前部件：前肩环的后半圈压在胸甲上，不给每个点包一圈勾线）
  function ringBack(R, x, y, rx, own) { if (own !== 0) E.part(); ringDots(x, y, false, (X, Y, k) => px(E, R, X, Y, RUNE_DOT(k) ? M.rune : M.gold, RUNE_DOT(k) ? Math.max(1, Math.min(3, P.gem + 1)) : 2), rx); }
  function pauldronBack(R, x, y) {
    E.part(); const m = M.mith;
    run(E, R, y - 5, x - 2, x + 1, m, 0); run(E, R, y - 4, x - 2, x + 1, M.gold, 0);          // 第一层
    run(E, R, y - 3, x - 3, x + 1, m, 0); run(E, R, y - 2, x - 3, x + 1, M.gold, 0);          // 第二层（往外错 1 格）
    run(E, R, y - 1, x - 3, x, m, 0); run(E, R, y, x - 3, x, M.gold, 0);                      // 第三层（往里收，整体竖着往下叠）
    px(E, R, x - 1, y - 6, m, 0); px(E, R, x - 1, y - 7, m, 4);   // 顶上的尖刺：朝上伸出 2 格
    px(E, R, x - 2, y - 5, m, 4); px(E, R, x - 3, y - 3, m, 4); px(E, R, x - 3, y - 1, m, 4);   // 每层左上角受光
    ringDots(x - 1, y - 2, true, (X, Y, k) => px(E, R, X, Y, RUNE_DOT(k) ? M.rune : M.gold, RUNE_DOT(k) ? Math.max(2, Math.min(4, P.gem + 2)) : 4), 4);
  }
  function pauldron(R, x, y, m, dir) {
    ...
    ringDots(x, y - 2, true, (X, Y, k) => px(E, R, X, Y, RUNE_DOT(k) ? M.rune : M.gold, RUNE_DOT(k) ? Math.max(2, Math.min(4, P.gem + 2)) : 4), 4);
  }
  // 候选部件：finHelm（秘银版）—— 鳍冠盔：闭口圆盔 + 金眉箍 + 面甲缝（里面是空的，只透出一对紫光）+ 口缝的金獠牙面饰 + 近侧金色獠牙形护颊；
  //   鳍冠高 6 格、齿尖包金，和盔体是同一个部件（散架时整顶头盔是一块，鳍冠不单独飞）。P.mist 1 = 面甲缝的紫光亮一档（喷紫雾）
  ...
  // 腰间垂下的锁甲裙（从金腰带下面一行开始）：逐行错开的 2 格链环、链环之间 1 格暗缝；左边 1 列受光（第 4 级）、右边 2 列背光（第 2 级），
  //   最上一行压在腰带阴影里（第 2 级），下摆按链环垂成一片片（暗缝处缺一格），越往下越宽
  function mailSkirt(R) {
    E.part(); const y0 = R.yWaist + 2, y1 = R.yHip + 3;
    for (let y = y0; y <= y1; y++) {
      const e = parts.edges(R, Math.min(y, R.yHip)), t = (y - y0) / Math.max(1, y1 - y0), L = e[0] - RD(t * 1.5), Rr = e[1] + RD(t);
      for (let x = L; x <= Rr; x++) {
        const seam = ((x + (y & 1) + 300) % 3) === 2;
        if (y === y1 && seam) continue;
        px(E, R, x, y, M.mail, x === L ? 4 : x >= Rr - 1 ? 2 : seam || y === y0 ? 2 : 3);
      }
    }
  }
  // 胸甲（接着 parts.torso 画，同一个部件，不另起分界线）：把板甲自带的中脊高光和胸腹分界抹平成整块秘银，只留一条暗色中缝；
  //   中缝上嵌胸口的符文核心（菱形 5 格，4 档亮度）。中缝放在头和盾之间露出来的那段胸甲正中（身体中线往后 3 格），不贴着前臂的分界线
  function chestPlate(R) {
    const e3 = parts.edges(R, R.yS + 3), x = RD((e3[0] + e3[1]) / 2) - 3, y = R.yS + 4, c = P.core;
    for (let yy = R.yS + 1; yy < R.yWaist; yy++) { const e = parts.edges(R, yy); run(E, R, yy, e[0], e[1], M.mith, 0); px(E, R, x, yy, M.mith, 2); }
    { const e = parts.edges(R, R.yWaist); for (let xx = e[0]; xx <= e[1] - 3; xx++) px(E, R, xx, R.yWaist, M.gold, xx === e[0] ? 4 : 3); }   // 金腰带用金的基色（自动明暗会整条亮成奶油白）；扣子那 2 格不动
  ...
  // 握盾的前臂：2 格宽的实心秘银（左列受光、右列背光）从肩甲下面垂到握把，最下面 1 格金手套握住盾的左沿；
  //   第 3 列伸到盾沿下面，压上盾以后只剩一条分界线（手臂和盾之间不夹胸甲）
  function shieldArm(R) {
    E.part(); const x0 = P.hx - 4, y0 = R.sFy + 1, y1 = P.hy - 1;
    for (let y = y0; y < y1; y++) { px(E, R, x0, y, M.mith, 4); px(E, R, x0 + 1, y, M.mith, 2); px(E, R, x0 + 2, y, M.mith, 2); }
    px(E, R, x0, y1, M.gold, 4); px(E, R, x0 + 1, y1, M.gold, 3); px(E, R, x0 + 2, y1, M.gold, 3);
  }
  // 候选部件：runeTower（秘银破魔版）—— 同一面符文塔盾升级：13×22，金框、上沿獠牙护角包金、外侧（右）翼形护边：上下两片 3 格宽的实心秘银翼片 + 外沿 1 格金边，
  //   上片 r 2–7、下片 r 9–14，中间隔 1 行缝；每片上沿平、下沿一格格往里收（3 3 3 2 2 1），像两片叠着的羽甲；
  //   （近身一侧的翼片压在前臂和胸甲上只会读成杂点，不画）、
  //   盾心一只立体的「破魔之眼」（金框杏眼 + 白眼 + 紫瞳；P.pupil 左右转、P.lid 0 睁 · 1 半闭 · 2 闭），眼两侧两列竖排符文、眼下一列（都带暗刻槽）；
  //   P.tilt 1 = 上掀（下沿往前 1、上沿往后 1）
  const WING = [0, 0, 3, 3, 3, 2, 2, 1, 0, 3, 3, 3, 2, 2, 1];                  // 每行翼片伸出的秘银格数（外面再包 1 格金边）
  function tower(R, cx, top) {
    ...
    for (let r = 0; r < h; r++) {
      ...
      if (w) { run(E, R, y, Rr + 1, Rr + w, M.shd, 0); px(E, R, Rr + w + 1, y, M.sgold, 0); }
    }
  ...
  // ───── 特效 ─────
  let pushT = 9, flipT = 9, chargeAcc = 0, chargeF = -1, chargeN = 0, soulAcc = 0, mistAcc = 0, lastStep = 0, lastDrop = 0;
  const wx = (x) => scrX(x), wy = (y) => HY + y;
  const HALO_C = () => [-4, -22];                                           // 死亡时符文环的圆心（本地坐标：K_SLUMP 姿、bx −2）
  function onEnter(s) {
  ...
  function onTime(s, t) {
    ...
    if (s === DEATH && t === T_BREAK) {                                   // 盔甲散架（死亡套件 parts）：头盔、肩甲、塔盾、胸甲各自飞出落地
      ...
      death.start('parts', DK_OPT); shake(0.16, 1);
      burst(wx(-2), wy(-16), 16, 30, 80, 0.3, 0.6, R_EL, 10);
    }
    if (s === DEATH && t === T_CLANK) { for (let i = 0; i < 14; i++) spawn(K_DUST, HX + HELM_LX - 9 + Math.random() * 18, HY - 1, (Math.random() - 0.5) * 30, -8 - Math.random() * 12, 0.4 + Math.random() * 0.4, FXI.dust); shake(0.1, 1); sfx('fall', { w: 0.9 }); }
  }
  ...
  function stepFX(dt, state, stT) {
    if (state === CHARGE) {                                             // 符文粒子被逆螺旋吸进眼里
      ...
      const f = Math.floor(stT * 12 + 1e-6); if (f !== chargeF) { chargeF = f; chargeAcc += 2; }   // 每个 12 fps 帧多生成 2 颗
      while (chargeAcc >= 1) {                                          // 汇聚半径 14–22 格（从盾外面开始吸），每 3 颗有一颗是 2×2 的大粒子，压在银白盾面上也看得见
        chargeAcc -= 1; chargeN++; const a = Math.random() * 6.2832, r = 14 + Math.random() * 8;
        spawnX(K_SPIRAL_PT, wx(EYE_CH[0]), wy(EYE_CH[1]), r / (0.35 + Math.random() * 0.3), 0, 9, R_EL, { a, r, w: -3 - Math.random() * 2, sz: chargeN % 3 === 0 ? 2 : 1 });
      }
    }
    ...
    if (state === DEATH && stT > T_BREAK + DK_OPT.fadeAt - 0.05 && stT < T_BREAK + DK_OPT.fadeAt + DK_OPT.fadeDur) { soulAcc += dt * 28; while (soulAcc >= 1) { soulAcc -= 1; spawn(K_RISE, HX - 12 + Math.random() * 22, HY - 1 - Math.random() * 8, (Math.random() - 0.5) * 6, -14 - Math.random() * 16, 0.8 + Math.random() * 0.8, FXI.soul); } }
    pushT += dt; flipT += dt;
  }
  function fxReset() { pushT = 9; flipT = 9; chargeAcc = 0; chargeF = -1; chargeN = 0; soulAcc = 0; mistAcc = 0; lastStep = 0; lastDrop = 0; }
  ...
  function fxFront(f12) {
    ...
    if (st === DEATH && t >= INCOMING + 0.3) {                           // 掉在地上的符文石板（死后和碎甲一起消散）
      const c = HALO_C(), fade = clamp01((t - T_BREAK - DK_OPT.fadeAt) / DK_OPT.fadeDur);
      for (let k = 0; k < 6; k++) {
