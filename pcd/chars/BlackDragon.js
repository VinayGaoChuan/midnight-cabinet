// 黑龙（部队 · 僵尸 · 先锋 · 神话）：升级线第 2 级（由赤龙升级）。飞行单位，一对巨膜翼压在半空悬停。
// 同一条龙长成了传说：赤龙的熔岩裂纹冷却成同样纹路的棱光晶脉（平时暗紫，一段青白的光从头流到尾）；近侧断角的断口上长出一截棱光水晶角，远侧角更长；
// 背上那对烂穿的破膜翼长成完整的巨翼（黑曜翼骨 + 暗紫翼膜），当年三个破洞的位置被发光的多边形晶片补上；尾端岩锤变成大一倍的棱晶锤；
// 下颌露骨处镶一圈黑曜石牙，胸前一块大棱晶护心镜。
// 攻击：悬停中身体一扭（转身），棱晶尾锤从身下往前横扫砸向目标，拖出 150° 扫击拖影弧。
// 技能「棱光护盾 + 过热」：双翼向前合拢包住身体蓄力 → 猛地张翼昂头，身前立起六边形晶面护盾 → 三发来袭攻击打在盾上被折射成三色光束弹开（盾只闪一段、不破）
// → 晶脉从青转成血红、外沿一圈血红轮廓光 → 护盾碎成晶屑飘落。
// 身体用 parts-beast 的 quad（躯干、颈、头、前腿、背刺）；巨膜翼 + 晶片、晶脉、晶角、晶锤尾、折起的后腿、护心镜、黑曜石牙是本模块画的（通用的标了「候选部件」）。
PCD.define('BlackDragon', (E) => {
  const { Sprite, begin, part, bake, ease, clamp01, q12, f12of, walkDemo, fxRamp, FXI, FXR, HY, FLOOR, INCOMING,
  ...
  // ───── 颜色、材质 ─────
  const C_OB = E.color('#15121e');                                                             // 黑曜鳞暗面（专属 1 色）
  const OBS = [0, C_OB, 8, 54];                                                                // 黑曜鳞：比夜空暗的一块黑（基色 8 ≈ 设定的 #262033），左上亮边是紫色光泽 54，靠亮边和晶脉从夜空里读出来
  ...
  const PRISM3 = [R_PC, R_PV, R_PP], HEAT_RIM = [58, 57, 57, 56, 55];                         // 过热：外沿一圈血红轮廓光（和赤龙同一套）
  const m = B.mats(E, { main: OBS, belly: 'stone', mane: [0, C_OB, 9, 54], wing: [0, 53, 54, 43], claw: 'bone', horn: 'bone', teeth: 'bone', eye: [0, 0, 22, 21], glow: [22, 22, 21, 21] });
  m.wing = E.defMat([0, 53, 54, 43], 1); m.wingFar = E.defMat([0, 52, 53, 54], 1);           // 翼膜：暗紫，比黑曜鳞亮一截（band 1，大块读得出面）；远翼暗一级
  m.hornFar = E.defMat([8, 7, 7, 6], 1);
  m.cry = E.defMat([42, 43, 22, 21], 1, 1);                                                    // 棱光水晶（发光体，平涂）：深紫 · 淡紫 · 青 · 白
  m.cryP = E.defMat([42, 43, 63, 21], 1, 1);                                                   // 粉面
  m.hot = E.defMat([55, 57, 58, 21], 1, 1);                                                    // 过热晶脉（血红）
  m.dead = E.defMat([0, 42, 52, 53], 1, 1);                                                    // 碎掉 / 熄灭的晶体
  m.obt = E.defMat([0, 0, C_OB, 8], 1);                                                        // 黑曜石牙

  const OX = -14;                                                                              // 身体整体后移：锚点落在身体前段（近战站位）
  const SHAPE = { len: 15, chest: 7, rump: 5.5, waist: 0.2, hump: 1.5, leg: 5, lw: 3, thigh: 3, farDx: -2, stride: 2.5, lift: 2, foot: 'claw',
    neck: 4.5, neckA: 0.85, neckW: 3.6, head: { type: 'dragon', w: 8, h: 6.5, snout: 6, snH: 4.2, tip: 0.85, teeth: 2, horn: null }, headA: 0.4,
    tail: 'long', tailLen: 18, tailW: 5, mane: 'ridge', maneLen: 2, fur: 0, pattern: null, lieLegs: 1 };
  const o = Q.shape(Object.assign({ m }, SHAPE));
  const WING = { span: 32, chord: 12, fingers: 3 };                                           // 赤龙的破翼 18 / 8 → 完整巨翼 32 / 12（同样三根翼指）
  // 翼姿 [臂角 a0, 翼尖扇到的角 aT, 收拢 fold, 后缘落点下移 bd]（角度从正后方量，+ 向上；> π/2 朝前）。bd：翼膜后缘贴到背上的那一点比翼根低几格（下扑时贴到胯上）
  const WP = [
    [0.8, -0.5, 0, 1],       // 0 上扬（扑翼最高：整片翼面斜立在背上）
    [0.55, -0.4, 0.05, 1],   // 1 半抬（往后倒成扇面，翼面还在背线以上）
    [0.05, -1.0, 0.05, 4],   // 2 下压（扑翼最低：扇面压到臀侧，翼尖离地约 10 格）
    [0.85, 0.2, 0.45, 1],    // 3 回收（半收，往上抬）
    [0.95, -0.45, 0, 1],     // 4 张开（施放：最大）
    [1.55, 3.35, 0.5, 1],    // 5 向前合拢（蓄力：翼从头顶往前弯下，包住胸口）
    [-0.2, -0.65, 0.45, 2],  // 6 垂落（倒地）
    [0.55, -0.4, 0.15, 1],   // 7 滑翔半抬
  ];
  const FAR_HIDE = [0, 1, 1, 0, 0, 0, 1, 1];                                                    // 往后平展 / 下压 / 垂落时远翼藏在身后，不画
  const FLAP = [0, 1, 2, 3], FLAP_LIFT = [4, 5, 6, 5];                                        // 扑翼 4 帧：上扬 → 半抬 → 下压 → 回收；下压后身体最高（起伏 2 格）
  // 尾姿 [根部角 a0, 卷曲 curl]。悬空的尾姿晶锤都离地 ≥ 5 格，只有倒地（5）贴地
  const TAILS = [
    [-0.5, 0.36],   // 0 悬停：往后下垂，尾锤往上勾
    [-2.45, -0.25], // 1 蓄势：尾巴从身下往前勾
    [0.02, 0.1],    // 2 横扫出手（转身后，尾锤正对目标）
    [0.5, 0.3],     // 3 扫过去（往后上方甩）
    [-0.8, 0.4],    // 4 低垂（受击、收尾）
    [-0.05, 0],     // 5 倒地（贴地摊直）
    [0.05, 0.5],    // 6 上卷到身后（蓄力：晶锤高举在背后）
    [-0.2, 0.35],   // 7 往后平伸（施放张翼）
  ];
  const TAIL_SW = 0.1;
  // 晶脉流光：vph = 光头所在的「轨道位置」，0 水晶角 → 1–2 颈 → 2–6 躯干（胸 → 臀）→ 6–9 尾 → 9.5 晶锤；VPH_OFF = 这一刻没有流光
  const VPH_OFF = 15, TR_CLUB = 9.5;

  const HX = 76, DUR = DEFAULT_DUR.slice(), hero = new Sprite(84, 62, 62, 56);
  ...
  const SPEC = Q.KEYS.concat(B.COMMON, [['wp', 0, 7], ['tsw', 0, 7], ['vph', 0, 15], ['vlv', 0, 3], ['heat', 0, 2], ['shat', 0, 1]]);
  const P = { wp: 0, tsw: 0, vph: VPH_OFF, vlv: 1, heat: 0, shat: 0 }; Q.reset(P);
  let rig = null;
  function tuck(rg) { for (const L of rg.legs) { L.F[0] = L.T[0] + (L.front ? 2.5 : -1.2); L.F[1] = L.T[1] + (L.front ? 4.2 : 4.8); } }   // 悬停：前爪垂在胸下（不去够地面）
  function mkRig() { rig = Q.rig(P, o); if (!rig.lie) tuck(rig); return rig; }
  P.lift = 5; P.pitch = 1; mkRig();
  const HIT_POINT = [R(rig.C1.x + rig.C1.r * 0.75 + OX), R(rig.C1.y)];

  // ───── 姿势 ─────
  const F_ALL = ['bx', 'crouch', 'pitch', 'head', 'jaw', 'tail', 'reach', 'lift', 'glow'];
  const REST = { bx: 0, crouch: 0, pitch: 1, head: 0, jaw: 0, tail: 0, reach: 0, lift: 5, glow: 0 };
  const pose = (p) => Object.assign({}, REST, p);
  const A_WIND = pose({ bx: -1, pitch: 3, head: -1, jaw: 1, lift: 6 });                     // 蓄势：前身抬起，尾巴从身下往前勾
  const A_STRIKE = pose({ bx: 2, pitch: 0, head: 1, jaw: 2, lift: 5 });
  const A_FOLLOW = pose({ bx: 2, pitch: 0, lift: 5, jaw: 1 });
  const C_WRAP = pose({ bx: -2, pitch: 0, head: 2, lift: 5, glow: 1 });                      // 蓄力：低头看护心镜，双翼往前合拢，尾巴卷到身后
  const S_OPEN = pose({ bx: -6, pitch: 3, head: -2, jaw: 2, lift: 5, glow: 2 });            // 施放：猛地张翼、昂头吼，身子往后一撑，身前让出护盾的位置
  const T_WIND = 1 / 12, T_STRIKE = 2 / 12, T_HIT = [1 / 12, 3 / 12, 5 / 12], T_SHATTER = 1 / 12, T_CRASH = INCOMING + 0.66, T_ASH = INCOMING + 1.2;
  const tmp = {};
  ...
  function idle(tq, f12) {
    apply(REST); const k = flapAt(f12, 4);                                                     // 慢速重扑翼：每个翼姿 4 帧（0.33 s）
    P.tail = [0, 1, 0, -1][k];
    const lp = tq % DUR[IDLE], lf = f12of(lp);
    P.vph = lf < 20 ? lf >> 1 : VPH_OFF;                                                       // 一段流光从水晶角沿晶脉流到尾锤（每 2 帧前进一段，1.6 s 走完）
    if (lp >= 1.6 - 1e-6 && lp < 2.0) { P.head = 2; P.gem = lp >= 1.75 && lp < 1.92 ? 1 : 0; }   // 待机个性：低头俯视，护心镜一闪
  }
  function poseAt(st, t, T) {
    const tq = q12(t), f12 = f12of(T);
    Q.reset(P); P.wp = 0; P.tsw = 0; P.vph = VPH_OFF; P.vlv = 1; P.heat = 0; P.shat = 0; P.lift = 5; P.pitch = 1;
    if (st === IDLE) idle(tq, f12);
    else if (st === MOVE) {
      apply(REST); const k = flapAt(f12of(tq), 2); P.tail = [1, 0, -1, 0][k]; P.head = k === 2 ? 1 : 0; P.vph = (f12of(tq) >> 1) % 12;   // 低空重扑翼（6 fps）
      const w = walkDemo(tq, 14, -1); P.flip = w.flip; P.mx = w.mx + (w.flip ? 2 * OX : 0);    // 转身绕身体中心
    } else if (st === ATTACK) {
      if (tq < T_STRIKE - 1e-6) {                                                              // 预兆 2 帧：前身抬起、翼上扬，尾巴从身下往前勾
        E.mix(tmp, REST, A_WIND, ease.out(clamp01((tq + 1 / 12) / (2 / 12))), F_ALL); apply(tmp); P.wp = tq < T_WIND - 1e-6 ? 3 : 0; P.tsw = tq < T_WIND - 1e-6 ? 4 : 1; P.rim = tq < T_WIND - 1e-6 ? 0 : 1;
      } else if (tq < 0.42) {                                                                  // 身体一扭（转身），尾锤横扫过去
        E.mix(tmp, A_STRIKE, A_FOLLOW, ease.out(clamp01((tq - T_STRIKE) / 0.2)), F_ALL); apply(tmp);
        P.flip = 1; P.mx = 2 * OX; P.wp = tq < 0.25 ? 1 : tq < 0.33 ? 2 : 3; P.tsw = tq < 0.25 ? 2 : 3; P.vlv = tq < 0.25 ? 2 : 1; P.rim = tq < 0.25 ? 1 : 0; if (tq >= 0.33) P.tail = 1;   // 延续：尾往后上方甩过去，翼跟着往上收
      } else { E.mix(tmp, A_FOLLOW, REST, ease.inOut(clamp01((tq - 0.42) / 0.33)), F_ALL); apply(tmp); P.wp = tq < 0.58 ? 3 : 7; P.tsw = tq < 0.58 ? 4 : 0; }
    } else if (st === CHARGE) {
      const q = ease.inOut(clamp01(tq / 0.6)); E.mix(tmp, REST, C_WRAP, q, F_ALL); apply(tmp);
      P.wp = q < 0.35 ? 3 : 5; P.tsw = q < 0.35 ? 4 : 6; P.vlv = tq < 0.45 ? 1 : tq < 0.9 ? 2 : 3; P.vph = tq < 0.45 ? f12of(tq) : f12 % 12;
      P.gem = tq < 0.45 ? 1 : (f12 & 1 ? 2 : 1); P.rim = tq < 0.45 ? 1 : tq < 0.9 ? 2 : 3;      // 晶面、晶脉逐档变亮，轮廓光 3 档
      if (tq >= 1.1) P.lift = f12 & 1 ? 5 : 4;                                                 // 蓄满：悬停发颤
    } else if (st === CAST) {
      E.mix(tmp, C_WRAP, S_OPEN, ease.out(clamp01(tq / 0.12)), F_ALL); apply(tmp);
      P.wp = 4; P.tsw = 7; P.gem = 3; P.vlv = 3; P.vph = f12 % 12; P.rim = 2; P.heat = tq >= T_HIT[2] - 1e-6 ? 1 : 0;
      ...
      if (q < 0.3) P.wp = 4; else P.wp = FLAP[(f12 >> 2) & 3]; P.tsw = q < 0.5 ? 7 : 0;
      P.heat = tq < 0.34 ? 2 : tq < 0.5 ? 1 : 0; P.gem = q < 0.35 ? 2 : q < 0.75 ? 1 : 0; P.vlv = q < 0.5 ? 2 : 1; P.vph = q < 0.5 ? f12 % 12 : VPH_OFF; P.rim = q < 0.5 ? 3 : q < 0.8 ? 2 : 0;
    } else if (st === HURT) {
      const h = tq - INCOMING;
      if (h < 0) idle(tq, f12);
      else if (h < 0.2) { apply(REST); P.bx = -2; P.eyes = 1; P.head = -1; P.pitch = 2; P.tail = 2; P.lift = 6; P.wp = 3; P.tsw = 4; P.flash = h < 1 / 12 ? 1 : 0; P.vlv = 2; }   // 翼一收、抬头、尾巴甩起
      else if (h < 0.35) { apply(REST); P.bx = -1; P.eyes = 1; P.tail = 1; P.wp = 7; }
      else idle(tq, f12);
    } else if (st === DEATH) {
      const d = tq - INCOMING;
      if (d < 0) idle(tq, f12);
      else if (d < 0.3) { apply(REST); P.bx = -2; P.eyes = 1; P.head = -1; P.pitch = 2; P.tail = 2; P.wp = 3; P.tsw = 4; P.lift = d < 0.15 ? 6 : 5; P.flash = d < 1 / 12 ? 1 : 0; P.vlv = f12 & 1 ? 3 : 0; }
      else if (d < 0.5) { apply(REST); P.bx = -3; P.lie = 1; P.pitch = -2; P.head = 2; P.eyes = 1; P.lift = 3; P.wp = 6; P.tsw = 4; P.jaw = 1; P.vlv = 0; }   // 失去护盾，机头朝下从空中栽下
      else {
        apply(REST); P.pitch = 0; P.bx = -3; P.lie = 2; P.eyes = 1; P.jaw = 1; P.lift = d < 0.58 ? 3 : d < 0.66 ? 1 : 0; P.wp = 6; P.tsw = 5; P.vlv = 0; P.tail = d < 0.9 ? 1 : 0;
    ...
    const mo = rig.mouth; P.mox = R(mo[0]) + OX + P.bx; P.moy = R(mo[1]);
    B.key(P, SPEC);
  }

  // ───── 画 ─────
  // 棱光晶脉（候选部件：glowCracks——和赤龙的熔岩裂纹同一张表：t 臀心 0 → 胸心 1，v 背线 0 → 腹线 1；段号 0 颈 · 1 胸 · 2 腰 · 3 臀 · 4 腿与尾）
  const CRACKS = [
  ...
  // 一格晶脉：tr = 轨道位置（见 VPH_OFF 注释）。平时压在深紫（隔几格一点淡紫），只有流光经过的那一小段亮成白 → 青，后面拖一格粉
  function veinTone(tr, k) {
    if (P.shat) return k === 0 ? 3 : 2;
    const d = P.vph - tr, lv = P.vlv;
    if (lv === 0) return k === 0 ? 2 : 1;
    if (lv >= 3) return Math.abs(d) < 0.8 && (k & 1) ? 4 : 3;                                 // 蓄满 / 施放：全身晶脉青亮，流光处隔点白
    if (lv === 2) return Math.abs(d) < 0.8 ? 3 : 2;                                           // 蓄力中段：整体亮一档（淡紫），流光处青
    if (d > -0.35 && d <= 0.35) return k & 1 ? 3 : 2; if (d > 0.35 && d <= 1.1) return k === 0 ? 3 : 2; if (d > 1.1 && d <= 1.8 && (k & 1)) return -3;   // 流光：青淡紫相间的一小段，后面隔点拖粉（平时不出现白）
    return 1;                                                                                   // 平时：一色深紫的脉
  }
  function veinDot(x, y, tr) {
    x = R(x); y = R(y); const k = ((x * 3 + y * 5) % 4 + 4) % 4;
    if (P.shat) { U.dot(E, x, y, m.dead, veinTone(tr, k)); return; }
    if (P.heat === 2 || (P.heat === 1 && tr < 4)) { U.dot(E, x, y, m.hot, k === 1 ? 2 : 3); return; }
    const t = veinTone(tr, k); U.dot(E, x, y, t < 0 ? m.cryP : (P.vlv >= 3 && k === 2 ? m.cryP : m.cry), t < 0 ? -t : t);
  }
  function bodyVeins() {
    ...
    for (const c of CRACKS) {
      for (let i = 1; i + 3 < c.length; i += 2) {
        ...
        for (let s = 0; s <= n; s++) {
          ...
          veinDot(x, cl(R(sp[0] + lerp(v0, v1, s / n) * (sp[1] - sp[0])), sp[0] + 1, sp[1] - 1), 2 + (1 - lerp(t0, t1, s / n)) * 3);   // 胸 2 → 臀 5.8
        }
      }
    }
    const NB = rig.NB, NT = rig.NT, L = Math.hypot(NT.x - NB.x, NT.y - NB.y) || 1, nx = (NT.y - NB.y) / L, ny = -(NT.x - NB.x) / L;
    for (let s = 0; s <= 6; s++) { const q = 0.1 + s * 0.13, w = (s & 1) ? 0.4 : -0.6; veinDot(lerp(NB.x, NT.x, q) + nx * w, lerp(NB.y, NT.y, q) + ny * w + 0.5, 2 - q); }   // 颈：头 1 → 颈根 2
  }
  function belly() {
  ...
  function legVein(i) { const L = rig.legs[i], T = L.T, F = L.F; for (let s = 0; s <= 2; s++) { const q = 0.25 + s * 0.2; veinDot(lerp(T[0], F[0], q) + ((s & 1) ? 0.5 : -0.4), lerp(T[1], F[1], q), L.front ? 2.8 : 5); } }
  // 候选部件：tuckLeg——悬停时折起的后腿（代替 quad.leg 的圆盘大腿）：大腿往前下收、小腿折回往后、脚掌垂下，三枚爪尖朝下勾。far 远侧暗一级
  const TUCK = new Float32Array(6);
  function tuckGeo(i) { const T = rig.legs[i].T, kx = T[0] + 2.6, ky = T[1] + 2.8, hx = kx - 3.8, hy = ky + 1.4; TUCK[0] = kx; TUCK[1] = ky; TUCK[2] = hx; TUCK[3] = hy; TUCK[4] = hx + 1.2; TUCK[5] = hy + 1.8; return TUCK; }
  function tuckHind(i) {
    part(); const L = rig.legs[i], far = L.far, mat = far ? m.far : m.limb, T = L.T, g = tuckGeo(i), c = m.claw;
    U.taper(E, T[0] - 1, T[1] - 1, g[0], g[1], 2.2, 1.3, mat, 0);                             // 大腿：水滴形，从髋往前下收到膝
    U.taper(E, g[0], g[1], g[2], g[3], 1.2, 0.8, mat, 0);                                       // 小腿折回往后
    U.seg(E, g[2], g[3], g[4], g[5], 1, mat, 0);                                                // 跖骨垂下
    U.dot(E, g[4] - 1, g[5] + 1, c, 2); U.dot(E, g[4] + 1, g[5] + 1, c, 3); U.dot(E, g[4] + 2, g[5], c, 3);   // 三枚爪尖
    if (!far) for (let s = 0; s <= 1; s++) veinDot(lerp(T[0] - 1, g[0], 0.35 + s * 0.35), lerp(T[1] - 1, g[1], 0.35 + s * 0.35), 5);
  }
  // 尾 + 棱晶锤（候选部件：clubTail / crystalClub——长尾按尾姿表走、背刺、尾端晶簇）
  const TP = new Float32Array(3 * 40), TAIL_N = 18;
  function tailWalk(rg, tsw, sway) {
    const n = TAIL_N, T = TAILS[tsw], lim = rg.lie ? -0.5 : -6; let x = rg.tail.x, y = rg.tail.y, a = T[0];   // 悬空时尾巴中线最低到离地 6 格
    for (let k = 0; k <= n; k++) {
      const q = k / n; TP[k * 3] = x; TP[k * 3 + 1] = y; TP[k * 3 + 2] = lerp(o.tailW * 0.5, 0.8, q);
      a += T[1] * q * 0.35 + sway * (0.25 + q) * 0.35; x -= Math.cos(a); y -= Math.sin(a); if (y > lim) { y = lim; a = 0; }
    }
    return n;
  }
  function drawTail() {
    part(); const n = tailWalk(rig, P.tsw, (P.tail | 0) * TAIL_SW);
    ...
    for (let k = 3; k < n - 2; k += 2) veinDot(TP[k * 3], TP[k * 3 + 1], 6 + (k / n) * 3.3);
  ...
  function drawClub(tip) {                                                                     // 多面水晶簇：一根主晶 + 两根侧晶，比赤龙的岩锤大一倍
    part(); const [x, y, ux, uy] = tip, nx = -uy, ny = ux, lo = rig.lie ? -0.5 : -5;
    const on = P.vlv >= 2 || Math.abs(P.vph - TR_CLUB) < 1.1, fill = on ? 3 : 2, lit = on ? 4 : 3, sh = on ? 2 : 1;   // 平时是淡紫晶簇，流光到尾端 / 蓄力时亮成青白
    U.disc(E, x, y, 1.6, m.limb, 0);                                                           // 晶簇根部的黑曜石结
    const shard = (a, L, w, mat) => {
      const c = Math.cos(a), s = Math.sin(a), vx = ux * c - uy * s, vy = ux * s + uy * c, ex = x + vx * L, ey = Math.min(lo, y + vy * L);
      U.taper(E, x + vx, y + vy, ex, ey, w, 0.5, mat, fill);
      U.seg(E, x + vx - vy * 0.8, y + vy + vx * 0.8, ex - vy * 0.5, ey + vx * 0.5, 1, mat, vy * 0.8 + vx * 0.2 < 0 ? lit : sh);   // 受光面 / 背光面
      U.dot(E, ex, ey, mat, on ? 4 : 3);
    };
  ...
  // 巨膜翼（候选部件：prismWing——几何同 B.wing、和赤龙的破翼同一套；黑曜翼骨压在暗紫翼膜上，赤龙那三个破洞的位置补上发光的多边形晶片，碎掉后洞又露出来）
  function wingGeo(x, y, pw, w, g) {
    ...
    g.bx = x - w.chord * (1 - 0.3 * fold); g.by = y + (pw[3] || 1);
    if (!rig.lie) for (let k = 0; k < nf; k++) g.ty[k] = Math.min(g.ty[k], -5);                // 悬停时翼尖不碰地
    return g;
  }
  const GN = { tx: [0, 0, 0], ty: [0, 0, 0] }, GF = { tx: [0, 0, 0], ty: [0, 0, 0] }, GT = { tx: [0, 0, 0], ty: [0, 0, 0] }, POLY = [];
  const WING_F = { span: 30, chord: 11, fingers: 3 };
  function holesOf(g, out) {                                                                   // 赤龙破洞的位置（同一个公式）
  ...
  const PLATE = ['.a.', 'abc', 'bcc', '.c.'];                                                  // 多边形晶片：左上受光面（a）· 主面（b）· 背光面（c）
  function drawWing(g, far) {
    part(); const mem = far ? m.wingFar : m.wing, bone = far ? m.far : m.limb, nf = g.n;
    POLY.length = 0; POLY.push(g.x, g.y, g.wx, g.wy);
    for (let k = 0; k < nf; k++) {
      POLY.push(g.tx[k], g.ty[k]);
      const nx = k < nf - 1 ? g.tx[k + 1] : g.bx, ny = k < nf - 1 ? g.ty[k + 1] : g.by, dq = k < nf - 1 ? 0.18 : 0.42;   // 扇贝后缘：两指之间往腕部凹进去
      POLY.push(lerp((g.tx[k] + nx) / 2, g.wx, dq), lerp((g.ty[k] + ny) / 2, g.wy, dq));
    }
    POLY.push(g.bx, g.by); U.poly(E, POLY, mem, 0);
    if (!far) for (let k = 0; k < nf; k++) U.seg(E, g.wx, g.wy, lerp(g.wx, g.tx[k], 0.92), lerp(g.wy, g.ty[k], 0.92), 1, bone, k === 0 ? 3 : 2);   // 翼指：黑曜骨线（远翼只画膜和臂骨，少一层线）
    U.seg(E, g.x, g.y, g.wx, g.wy, 2, bone, 0);                                                 // 臂骨（前缘）
    U.dot(E, g.wx, g.wy - 1, m.claw, 3); U.dot(E, g.wx + 1, g.wy - 1, m.claw, 4);              // 拇指爪
    if (far) return;
    holesOf(g, HOLES);                                                                          // 晶片和翼膜是同一个部件：直接嵌在膜里，不压分界线
    for (let h = 0; h < 3; h++) {
      const x0 = R(HOLES[h * 2]) - 1, y0 = R(HOLES[h * 2 + 1]) - 1;
      if (P.shat) { U.dot(E, x0 + 1, y0 + 1, 0); U.dot(E, x0 + 1, y0 + 2, 0); U.dot(E, x0 + 2, y0 + 2, 0); continue; }   // 晶片碎了：赤龙时的破洞又露出来
      const lv = P.gem >= 2 || P.vlv >= 3 ? 1 : 0, mat = h === 1 ? m.cryP : m.cry;
      for (let j = 0; j < 4; j++) for (let i = 0; i < 3; i++) {
        const ch = PLATE[j][i]; if (ch === '.') continue;
        U.dot(E, x0 + i, y0 + j, ch === 'c' ? m.cry : mat, ch === 'a' ? 3 + lv : ch === 'b' ? 2 + lv : 1 + lv);
      }
    }
  }
  // 胸前棱晶护心镜（发光体，单独部件）
  ...
  function drawMirror() {
    ...
    for (let j = 0; j < 6; j++) for (let i = 0; i < 5; i++) {
      ...
      let mat = m.cry, t = ch === 'a' ? (g >= 1 || j === 0 ? 4 : 3) : ch === 'b' ? (g >= 2 && ((i + j) & 1) ? 4 : 3) : (g >= 3 ? 3 : 2);
  ...
  function nearHorn() {
    ...
    part(); const lit = P.gem >= 2 || P.vlv >= 3, flow = Math.abs(P.vph) < 1.1;                // 断口上长出一截棱光水晶角：平时淡紫 / 青，流光起点 / 蓄力时亮成白
    const px = [[0, -2, 3], [-1, -2, 4], [-1, -3, 3], [-2, -3, 4], [-2, -4, 3], [-3, -4, 4], [-3, -5, 2], [-4, -5, 3], [-5, -6, 4]];
    for (const [dx, dy, t] of px) U.dot(E, x0 + dx, y0 + dy, t === 2 ? m.cryP : m.cry, lit || flow ? t : (t === 4 ? 3 : 2));
  }
  function obsidianJaw() {                                                                     // 和头同一个部件：下颌露骨处镶一圈黑曜石牙
    const F = Q.headFrame(rig, o, P.jaw);
    for (let s = 0; s < 5; s++) { const u = F.uc + 0.4 + s, vb = F.prof(u)[1] + F.gap(u), p = F.at(u, vb - 0.1); U.dot(E, p[0], p[1], (s & 1) ? m.obt : m.teeth, (s & 1) ? 2 : 3); }
    const t1 = F.at(F.uT - 1.2, F.prof(F.uT - 1.2)[2] + 0.9); if (!P.jaw) { U.dot(E, t1[0], t1[1], m.obt, 3); U.dot(E, t1[0], t1[1] - 1, m.cry, P.shat || P.vlv < 2 ? 2 : 3); }
  }
  function drawHero() {
    begin(hero, P.bx + OX, 0);
    const lie = rig.lie === 2, wp = WP[P.wp];
    wingGeo(rig.wing.x + 2, rig.wing.y - 1, [wp[0] + 0.1, wp[1] + 0.1, wp[2], wp[3]], WING_F, GF);  // 远翼：往前错 2 格、稍高，从近翼上沿探出一截
    wingGeo(rig.wing.x, rig.wing.y, wp, WING, GN);
    if (!FAR_HIDE[P.wp]) drawWing(GF, 1);
    if (!lie) { tuckHind(0); Q.leg(E, rig, P, o, 1); }
    const front = P.tsw === 1;                                                                  // 尾从身下往前勾时画在身体前面
    let tip = null;
    if (!front) { tip = drawTail(); drawClub(tip); }
    Q.ridge(E, rig, P, o);
    Q.body(E, rig, P, o); belly(); bodyVeins();
    const glowThru = P.wp === 5;                                                                // 双翼合拢包住胸口时，护心镜的光透过翼膜（画在近翼之后）
    if (!glowThru) drawMirror();
    if (!lie) { tuckHind(2); Q.leg(E, rig, P, o, 3); legVein(3); }
    if (front) { tip = drawTail(); drawClub(tip); }
    drawWing(GN, 0);
    if (glowThru) drawMirror();
    farHorn();
  ...
  // ───── 特效 ─────
  const HXC = HX + 9, HYC = HY - 18, HH = 14, HW = 4;                                          // 六边形晶面护盾（屏幕坐标）：立在昂起的龙头前、假人手臂前
  const HEX = [HXC, HYC - HH, HXC + HW, HYC - HH / 2, HXC + HW, HYC + HH / 2, HXC, HYC + HH, HXC - HW, HYC + HH / 2, HXC - HW, HYC - HH / 2];
  const HIT_Y = [HYC - 3, HYC - 9, HYC + 3], SHOT_VX = 110, SHOT_VY = 176;                    // 敌弹从右上方斜着落下（越过假人头顶），打在盾的右上沿和右沿
  const REFRACT = [[-65, R_PC], [-10, R_PV], [45, R_PP]], BEAM_L = 7;                          // 折射光束：往上 / 往右 / 往下，三道各 7 格
  const hitX = (y) => (Math.abs(y - HYC) <= HH / 2 ? HXC + HW : HXC + HW * (1 - (Math.abs(y - HYC) - HH / 2) / (HH / 2)));
  const hitT = [9, 9, 9];
  let shieldT = 9, chargeAcc = 0, soulAcc = 0, lastGf = -9, hfT = 9;
  ...
  function onEnter(s) {
    if (s === CAST) {
      ...
      for (let k = 0; k < 3; k++) { const y = HIT_Y[k], x = hitX(y); shoot(3, x + SHOT_VX * T_HIT[k], y - SHOT_VY * T_HIT[k], -SHOT_VX, x, FXI.enemy, SHOT_VY, { trail: { back: [2, 8], off: 1 } }); }   // 三发来袭攻击，正好在挡击帧打上盾面
  ...
  function onTime(s, t) {
    ...
    if (s === CAST && T_HIT.includes(t)) {                                                     // 折射：三道不同颜色的光束往上 / 右 / 下弹开 + 白十字，盾面只闪一段
      const k = T_HIT.indexOf(t), y = HIT_Y[k], x = R(hitX(y)); hitT[k] = 0;
      for (const [deg, rp] of REFRACT) { const a = deg / 180 * PI; fx.beam(x + 1, y, x + 1 + R(Math.cos(a) * BEAM_L), y + R(Math.sin(a) * BEAM_L), 1, rp, 0.3, 2); }
  ...
  function stepFX(dt, state, stT) {
    ...
    shieldT += dt; hfT += dt; hitT[0] += dt; hitT[1] += dt; hitT[2] += dt;
  }
  function fxReset() { shieldT = 9; hfT = 9; hitT[0] = hitT[1] = hitT[2] = 9; chargeAcc = 0; soulAcc = 0; lastGf = -9; }
  function fxBack(f12) {
