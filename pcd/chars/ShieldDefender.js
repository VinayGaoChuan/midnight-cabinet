...
// 护鼻尖顶盔 + 锁子护颈（斜肩），赭黄菱格绗缝棉甲，后手一柄翼片钉头锤（锤头从盾上沿后面露出来）。
// 攻击 = 砸：锤从盾后抬起、越过盾上沿短促下砸，前冲 2 格，盾不离身；技能 = 特性「偏转」生效：缩到盾后斜举鸢盾，盾前亮起一道 45° 的青铜偏转面，
...
PCD.define('ShieldDefender', (E) => {
  ...
  const M = parts.mats(E, {
    gamb: { r: 'sand', band: 2 }, sleeve: 'sand', pants: 'stone', boot: 'boot', belt: 'leather', buckle: 'iron',
    skin: 'skin', ink: { r: 'ink', flat: 1 }, mail: 'steel', helm: 'steel', iron: 'iron', wood: 'wood', hair: [20, 19, 16, 33],
    face: 'sand', band: 'iron', fletch: 'white', shaft: 'wood',
  ...
  const SH = { W: 4, H: 15, rc: 6 };                                                  // 鸢盾：半宽 4（9 格宽）× 15 行，盾心在第 6 行
  ...
  // ───── 姿势：前手 = 盾（hx hy，盾上沿中点 = 手 + (1, −7)），后手 = 锤（bhx bhy ba）─────
  // 额外字段：mask 盾上的箭（位）· tilt 斜举档（正 = 上沿往后仰，每档每行 0.22 格）· pk 待机拔箭帧（0 = 没有，1–12）· mfree 锤靠在肩上、手不握 · shd 死亡时盾（0 在手上 · 1 往前倾 · 2 快倒平 · 3 平拍在地）
  const P = { hx: 0, hy: 0, a: 0, bhx: 0, bhy: 0, ba: 0, lean: 0, head: 0, crouch: 0, bob: 0, bx: 0, step: 0, wup: 0, walk: 0, beard: 0, sway: 0, bend: 0,
    gem: 0, glint: 0, rim: 0, eyes: 0, flash: 0, lying: 0, lift: 0, hatX: 0, hatY: 0, dq: 0, st: 0, mask: 7, tilt: 0, pk: 0, mfree: 0, shd: 0, gx: 0, gy: 0, flip: 0, mx: 0, k1: 0, k2: 0 };
  const K = (hx, hy, bhx, bhy, ba, lean, head, crouch) => ({ hx, hy, bhx, bhy, ba, lean: lean || 0, head: head || 0, crouch: crouch || 0 });
  const K_IDLE = K(7, -8, 8, -9, -0.15);                        // 盾从下巴挡到脚踝，锤竖在盾后、锤头从盾上沿露出
  const K_WIND = K(6, -9, 1, -17, -0.5, -1, 0, 1);              // 锤从盾后抬到脑后
  const K_STRIKE = K(8, -8, 6, -17, 1.9, 1, 1);                 // 越过盾上沿短促下砸
  const K_HOLD = K(8, -8, 6, -15, 2.2, 1);
  const K_BRACE = K(8, -14, 8, -10, -0.15, 0, 0, 2);            // 蓄力：矮身缩到盾后，盾抬到齐眼
  const K_PUSH = K(9, -14, 8, -10, -0.15, 1, 0, 2);             // 施放：盾往前推 1 格
  const K_HURT = K(5, -9, 7, -10, -0.35, -1, -1);
  ...
  const KEY2 = parts.keyer([['step', -1, 1], ['wup', 0, 2], ['walk', 0, 1], ['beard', -3, 3], ['sway', -2, 2], ['gem', 0, 4], ['rim', 0, 3], ['eyes', 0, 1], ['flash', 0, 1],
    ['lying', 0, 1], ['lift', 0, 3], ['hatX', 0, 15], ['hatY', 0, 7], ['dq', 0, 48, 48], ['st', 0, 8], ['mask', 0, 7], ['tilt', -3, 3], ['pk', 0, 12], ['mfree', 0, 1], ['shd', 0, 3]]);
  const T_STRIKE = 2 / 12, T_SHD = INCOMING + 0.18, T_LAND = INCOMING + 0.66, T_PUFF = 0.3;
  const T_HIT = [0.07, 0.2, 0.35];                               // 施放段里三支箭撞上偏转面的时刻
  // 待机个性「拔箭」（1.4–2.4 s，每 1/12 s 一格）：[后手 x, y, 盾上的箭, 手里箭的方向（-1 没有）, 头]
  const PLUCK = [[11, -18, 7, -1, 0], [10, -20, 6, 2, 0], [6, -21, 6, 0, 0], [5, -20, 6, 4, 1], [5, -20, 6, 4, 1], [1, -24, 6, 14, -1],
    [-3, -23, 6, -1, -1], [-1, -17, 6, -1, 0], [4, -12, 6, -1, 0], [8, -9, 6, -1, 0], [8, -9, 6, -1, 0], [8, -9, 6, -1, 0]];
  const FLY = { 6: [-6, -25, 13], 7: [-10, -26, 12], 8: [-13, -22, 10], 9: [-15, -14, 9], 10: [-17, 0, 9], 11: [-17, 0, 9] };   // 扔出去的断箭：[箭头 x, y, 方向]，第 10 格插进地里
  const PK0 = 1.4;
  ...
  function poseAt(st, t, T) {
    ...
    P.hatX = 0; P.hatY = 0; P.dq = 0; P.bob = 0; P.flip = 0; P.mx = 0; P.mask = 7; P.tilt = 0; P.pk = 0; P.mfree = 0; P.shd = 0;
    ...
    } else if (st === DEATH) {                                       // 受击 → 踉跄、盾脱手往前倒平 → 人扑在盾上 → 尖盔滚出去 → 铜钉闪两下熄灭 → 消散
      const d = tq - INCOMING; P.rim = 0;
      if (d < 0) { idle(tq, f12); P.rim = 1; }
      else {
        P.pk = 0; P.mfree = 0; P.mask = 7;
        if (d < 0.3) { setK(K_HURT, K_HURT, 0); P.bx = -2; P.eyes = 1; P.beard = 3; P.sway = 1; P.flash = d < 1 / 12 ? 1 : 0; P.crouch = d < 0.15 ? 0 : 1; P.gem = (f12 & 1) ? 1 : 0; }
        else if (d < 0.5) { setK(K_STAG, K_STAG, 0); P.eyes = 1; P.beard = 1; P.shd = d < 0.38 ? 1 : d < T_SHD - INCOMING + 0.1 ? 2 : 3; P.mask = P.shd === 3 ? 4 : 7; P.gem = (f12 & 1) ? 1 : 0; }
        else {
          setK(K_DOWN, K_DOWN, 0); P.lying = 1; P.eyes = 1; P.shd = 3; P.mask = 4; P.lift = d < 0.58 ? 3 : d < 0.66 ? 1 : 0;
          const hq = clamp01((d - 0.66) / 0.3); P.hatX = RD(9 * hq); P.hatY = RD(Math.sin(hq * Math.PI) * 4);
          P.gem = d < 0.66 ? 1 : d < 0.74 ? 2 : d < 0.82 ? 4 : d < 0.9 ? 2 : 4;
  ...
  // ───── 本角色的部件（通用的标「候选部件」）─────
  // 候选部件：kiteShield —— 可调尺寸的鸢盾（上沿一行收 1 格、上宽下尖，尖端 2 行单格），盾面朝镜头挂在前臂上。
  //   o = { W 半宽, H 高, rc 盾心行, face 盾面, rim 铁包边, band 斜带材质（左上 → 右下 2 格宽，和盾面同一部件）, boss / bglow 盾心钉（flat，按 lv 5 档亮：0 待机 · 1 蓄力 · 2 蓄满 · 3 施放 · 4 熄灭）,
  ...
  function kiteHalf(W, H) {
    ...
    for (let r = 0; r < H; r++) a.push(r === 0 ? W - 1 : r <= kn ? W : RD(W * (1 - Math.pow((r - kn) / (H - 1 - kn), 0.85))));
    return (KH[key] = a);
  }
  function kiteShield(T, cx, top, o) {
    const W = o.W, H = o.H, hw = kiteHalf(W, H), tl = o.tilt || 0, sh = (r) => RD(tl * (r - H / 2)), slope = (2 * W - 2) / (H * 0.6 - 1);
    E.part();
    for (let r = 0; r < H; r++) {
      const w = hw[r], y = top + r; if (o.clip != null && y > o.clip) continue;
      const s = sh(r), b0 = Math.floor(-W + 1 + (r - 1) * slope);
      for (let x = -w; x <= w; x++) {
        const up = r > 0 ? hw[r - 1] : -1, dn = r < H - 1 ? hw[r + 1] : -1, rim = r === 0 || r === H - 1 || Math.abs(x) === w || Math.abs(x) > up || Math.abs(x) > dn;
        let m = o.face, t = 0;
        if (rim) { m = o.rim; if (r === 0 && x < 0) t = 4; }
        else if (o.band && r > 0 && (x === b0 || x === b0 + 1)) m = o.band;
        px(E, T, cx + x + s, y, m, t);
      }
      if (!o.band || r < 2 || r > H * 0.5) continue;
      if (w >= 3 && b0 - 1 > -w) px(E, T, cx + b0 - 1 + s, y, o.face, 4);                        // 斜带左上沿一格亮边（漆面的光）
    }
  ...
  // 候选部件：kiteFlat —— 平拍在地上的鸢盾：侧看 2 行（上沿铁包边 + 漆面），尖端在前、只剩 1 行；盾心钉鼓出 1 格（按 lv 亮 / 熄灭），一截断箭茬朝上。x0 = 盾顶（宽的一端）
  function kiteFlat(x0, len, lv) {
    E.part(); const T = parts.FREE, bi = RD(len * 0.4);
    for (let i = 0; i < len; i++) {
      const x = x0 + i, tip = i >= len - 2, band = i === RD(len * 0.55) || i === RD(len * 0.55) + 1;
      px(E, T, x, 0, M.iron, i === 0 ? 4 : 0);
      if (!tip) px(E, T, x, -1, i === 0 || i === len - 3 ? M.iron : band ? M.band : M.face, i === 0 ? 4 : 0);
    }
    px(E, T, x0 + bi, -1, lv === 4 ? M.boss : lv >= 2 ? M.bglow : M.boss, lv === 4 ? 1 : lv >= 2 ? 3 : 4); px(E, T, x0 + bi, -2, M.boss, lv === 4 ? 1 : 2);
    px(E, T, x0 + 3, -2, M.shaft, 3); px(E, T, x0 + 4, -3, M.shaft, 4);                     // 没折断的那截箭茬
    return [x0 + bi, -1];
  }
  ...
  function shieldGeo() { return { cx: P.hx + 1, top: P.hy - 7, tilt: P.tilt * 0.22 }; }
  function bossAt() {
    if (P.shd === 3) return [FLAT_X0 + RD(FLAT_LEN * 0.4), -1];
  ...
  const FLAT_X0 = 5, FLAT_LEN = 15;
  function dropGeo() { return P.shd === 1 ? { cx: 11, top: -14, tilt: -0.45, H: SH.H, rc: SH.rc } : { cx: 13, top: -7, tilt: -0.9, H: 8, rc: 3 }; }   // 盾往前倒：1 = 尖着地、上沿前倾 · 2 = 快拍平（压扁成 8 行）

  // 只给已经画成材质 m 的格子改色调（绗缝线、锁子环纹这种「在同一部件里补纹理」用，不会在部件外面多出像素）
  function retone(R, x, y, m, t) { const s = parts.toSprite(R, x, y), X = s[0] + P.bx + hero.ox, Y = s[1] - P.lift + hero.oy; if (X < 0 || Y < 0 || X >= hero.w || Y >= hero.h) return; if (hero.mat[Y * hero.w + X] === m) px(E, R, x, y, m, t); }

  // ───── 画（部件从后往前）─────
  function drawHero() {
    E.begin(hero, P.bx, -P.lift); const R = parts.rig(P, BODY), dead = P.st === DEATH;
    ...
    parts.arm(E, R, P, { side: 'B', sleeve: 'loose', mat: M.sleeveD, cuff: M.beltD, cuffStyle: 'bracer', grip: 'none', at: R.lie ? [R.sBx + 2, R.yWaist] : null });
    ...
    { const [LL, RR] = tor.rows; for (let i = 2; i < LL.length - 1; i++) { const y = tor.y0 + i; for (let x = LL[i] + 1; x < RR[i]; x++) if ((((x + y) % 4) + 4) % 4 === 0 && (y & 1) === 0) retone(R, x, y, M.gamb, 2); } }   // 菱格绗缝（和棉甲同一部件：暗色隔点）
    parts.head(E, R, P, { mat: M.skin, face: 'square', eye: M.ink, nose: 'big', mouth: 'line', ear: 'none' });
    parts.mantle(E, R, P, { style: 'plain', mat: M.mail, len: 3 });                           // 锁子护颈：从盔檐垂到肩上
    { const x0 = R.hx0; for (let y = R.ey; y < R.yS; y++) { px(E, R, x0 - 1, y, M.mail, 0); if (y > R.ey) px(E, R, x0, y, M.mail, 0); }
      for (let y = R.ey; y < R.yS + 3; y++) for (let x = x0 - 4; x <= R.hx1 + 4; x++) if ((((x + 2 * y) % 4) + 4) % 4 === 1) retone(R, x, y, M.mail, 2); }   // 后脑的锁子 + 环纹（只改已经是锁子的格子）
    const helm = { style: 'nasal', mat: M.helm, trim: M.iron };
    if (R.lie) parts.helm(E, R, P, Object.assign({ at: [24 + P.hatX, -2 - P.hatY], rot: (P.hatX >> 2) & 3 }, helm));   // 尖盔从头上滚出去
    else parts.helm(E, R, P, helm);
    const lift = P.st === ATTACK || P.st === CHARGE || P.st === CAST || P.st === RECOVER;
    if (!dead || (!P.shd && !R.lie)) {
      if (P.mfree) parts.hammer(E, R, P, Object.assign({}, MACE, { at: [K_IDLE.bhx, K_IDLE.bhy + P.bob], a: K_IDLE.ba }));   // 锤靠在肩上，手去拔箭
      else parts.hammer(E, R, P, MACE);
    ...
    const SO = { W: SH.W, H: SH.H, rc: SH.rc, face: M.face, rim: M.iron, band: M.band, boss: M.boss, bglow: M.bglow, lv: P.gem, arrows: ARROWS, mask: P.mask, shaft: M.shaft, fletch: M.fletch, wob: Math.max(-1, Math.min(1, -P.beard)) };
  ...
  function bakeHero() { RIM.rim = P.rim; RIM.rx = P.gx + hero.ox; RIM.ry = P.gy + hero.oy - P.lift; RIM.flash = P.flash; RIM.dq = P.dq; bake(hero, RIM); }
  ...
  const PL = [3, -30], PLN = 12, HITK = [10, 6, 2];              // 偏转面：盾面前上方，从 (3, −30) 斜到 (15, −18)（45°），三支箭依次撞在第 10、6、2 格
  ...
  function onEnter(s) {
    ...
    const bx = wx(P.gx), by = wy(P.gy); burst(bx, by, 14, 40, 100, 0.25, 0.55, R_EL, 10); fx.cross(bx, by, 5, R_EL, 0.3);
    shake(0.28, 2); flash(0.05);
  }
  function deflect(i) {                                            // 第 i 支箭撞上偏转面：改道 + 火花 + 青铜外爆 + 星芒
    const x = wx(PL[0] + HITK[i]), y = wy(PL[1] + HITK[i]); hitT[i] = 0; aX[i] = x; aY[i] = y; aSt[i] = 2; aT[i] = 0; aTr[i] = 0.22;
    if (i === 0) { aVX[i] = 40; aVY[i] = 230; aG[i] = 0; aFloor[i] = HY + 1; }               // 向下插进地里
    else if (i === 1) {                                                                         // 向上翻着弹飞，收招时掉回来插在盾上
      const g = restGeo(), c = [g.cx + ARROWS[0][0], g.top + ARROWS[0][1]], T = 0.3 + T_PUFF, tx = wx(c[0]), ty = wy(c[1]);
      aG[i] = 520; aVX[i] = (tx - x) / T; aVY[i] = (ty - y - 0.5 * aG[i] * T * T) / T; aSpin[i] = 1; aTr[i] = 0.3;
    } else { aVX[i] = -230; aVY[i] = -85; aG[i] = 0; }                                          // 擦过头顶往左上飞走
  ...
  function onTime(s, t) {
    ...
    if (s === DEATH && Math.abs(t - T_SHD) < 1e-9) {                 // 盾平拍在地上：扬尘，两支断箭折断弹起
      ...
      for (let i = 0; i < 6; i++) spawnX(K_PHYS, wx(8 + i * 1.5), HY - 3, (Math.random() - 0.3) * 40, -60 - Math.random() * 40, 0.7 + Math.random() * 0.3, R_SPL, { g: 260, floor: HY });
      sfx('fall', { w: 0.35 });
    }
  ...
  function stepFX(dt, state, stT) {
    ...
    if (state === CHARGE) {                                          // 青铜光点从盾沿四周汇聚到盾心铜钉
      ...
      while (chargeAcc >= 1) { chargeAcc -= 1; const a = Math.random() * 6.2832, r = 8 + Math.random() * 6; spawn(K_SPIRAL_PT, bx, by, r / (0.3 + Math.random() * 0.3), 0, 9, R_EL, a, r, (Math.random() - 0.5) * 2); }
    }
    ...
    if (state === IDLE) { if (P.pk === 11 && lastPk !== 11) spawn(K_DUST, wx(-17), HY, (Math.random() - 0.5) * 10, -6, 0.35, FXI.dust); lastPk = P.pk; }   // 扔掉的断箭落地：1 颗尘
    ...
    for (let i = 0; i < AN; i++) {
      ...
      if (aTr[i] > 0) { aTr[i] -= dt; spawn(K_STILL, aX[i], aY[i], 0, 0, 0.25, R_EL); }
  ...
  function fxFront(f12) {
    if (plT < 1.1) {                                               // 偏转面：45° 斜的青铜点阵，每 2 格一点、逐点亮起；撞击处闪白 2 帧，最后一下整片闪白，收招时断续熄灭
      const lit = clamp01(plT / 0.1), gone = clamp01((plT - 0.55) / 0.5);
      for (let k = 0; k <= PLN; k += 2) {
        if (k / PLN > lit + 1e-6 || hash(k, 5) < gone) continue;
        let c = ((k >> 1) + f12) % 3 === 0 ? EL[1] : EL[2];
        for (let i = 0; i < 3; i++) if (hitT[i] < 2 / 12 && Math.abs(k - HITK[i]) <= 2) c = EL[0];
        if (plWhite < 2 / 12) c = EL[0];
        const x = wx(PL[0] + k), y = wy(PL[1] + k); put(x, y, c); put(x + 1, y, gone > 0 ? EL[3] : EL[2]); if (k / PLN > lit - 0.12) put(x, y - 1, EL[0]);
      }
    }
