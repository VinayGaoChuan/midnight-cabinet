// 星辉巫师（范式角色）：瘦高老法师、折角尖帽、长白胡、喇叭袖长袍、杖顶宝石；远程甩杖奥术弹；技能「星辉爆」。
PCD.define('star-wizard', (E) => {
  const { defMat, Sprite, begin, part, sp, run, rect, line, brush, bake, ease, clamp01, q12, f12of, gait, walkDemo, FXI, FXR, HY, DUMMY_X, INCOMING, ASTEP,
    IDLE, MOVE, ATTACK, CHARGE, CAST, RECOVER, HURT, DEATH, REVIVE, DEFAULT_DUR, K_SPIRAL, K_EMBER, K_RISE, K_DUST,
    spawn, burst, releaseOrbit, shoot, ring, shake, flash, hitDummy, put, scrX, floorGlow, shotFloorGlow, sfx } = E;
  ...
  function onEnter(s) {
    ...
    if (s === CAST) { releaseOrbit(45, 100, 0.35, 0.75); burst(gx, gy, 30, 70, 140, 0.3, 0.75, R_EL, 10); shoot(2, gx + 2, gy, 120, DUMMY_X - 4); ring(gx, gy, 1, R_EL); shake(0.28, 2); flash(0.05); }
  }
  function onTime(s, t) {
    ...
    if (s === DEATH && t === INCOMING + 0.66) { for (let i = 0; i < 16; i++) { const x = HX - 20 + Math.random() * 24; spawn(K_DUST, x, HY - 1, (Math.random() - 0.5) * 30, -8 - Math.random() * 14, 0.4 + Math.random() * 0.4, FXI.dust); } shake(0.1, 1); sfx('fall', { w: 0.4 }); }
  }
  const EVENTS = [[], [], [T_FLICK], [], [], [], [], [INCOMING + 0.66], []];
  function impactOn(k, x, y) {
    if (k === 1) { burst(x, y, 8, 40, 90, 0.15, 0.35, R_EL, 10); hitDummy(0); sfx('hit', { mat: 'magic', w: 0.3 }); }
    else if (k === 2) { burst(x, y, 36, 60, 150, 0.3, 0.7, R_EL, 16); ring(x, y, 1, R_EL); hitDummy(1); shake(0.12, 1); sfx('impact', { pal: 'arcane', w: 0.8 }); }
  }
  function stepFX(dt, state, stT) {
    ...
    if (state === MOVE && P.step !== lastStep) { if (P.step !== 0) { sfx('step', { w: 0.3 }); } if (P.step !== 0) for (let i = 0; i < 2; i++) spawn(K_DUST, scrX(P.step > 0 ? 4 : -3) + (Math.random() - 0.5) * 3, HY, (Math.random() - 0.5) * 16, -4 - Math.random() * 6, 0.3 + Math.random() * 0.2, FXI.dust); lastStep = P.step; }
  ...
  return {
    name: '星辉巫师', HX, R_EL, DUR, hero, P, GLOW_MATS: [M_GEM, M_GLOW], HIT_POINT: [2, -13], EVENTS,
    // 音效声明：受击 / 死亡的身体材质、死亡方式、技能元素与蓄力花样、施放重量（charge / release / hurt / death 由引擎自动发）
    SFX: { body: 'flesh', how: 'topple', pal: 'arcane', style: 'spiral', w: 0.7 },
