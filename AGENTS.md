...
4. **角色与技能**：角色是 `pcd/` 里的像素角色模块（`pcd/chars/<key>.js`，规范见 pixel-character-design 技能），改完运行 `node tools/pcd-pack.mjs` 重新生成 `src/mc-pcd.js`（生成文件不要手改）。还在用 16-bit 骨骼的单位改 `src/mc-px16-cast.js` / `src/mc-px16-fx.js` 后运行 `node tools/gen-prompts.js`，重新生成 `docs/prompts/`。
